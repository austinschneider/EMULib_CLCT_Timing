...
emu::step::Test::Test(const string& id, const string& group, const string& testParametersXML, const string& generalSettingsXML,
		const string& specialSettingsXML, const bool isFake, Logger* pLogger)
		: TestParameters(id, testParametersXML, pLogger), group_(group), isFake_(isFake), isToStop_(false), runNumber_(0), runStartTime_(
...
void emu::step::Test::createEndcap(const string& generalSettingsXML, const string& specialSettingsXML) {
	...
	valuesMap.clear();
	for(x = xpaths.begin(), v = values.begin(); x != xpaths.end() && v != values.end(); ++x, ++v)
		valuesMap[x->second] = v->second;
...
int emu::step::Test::getDDUInputFiberMask(int crateId, int dduSlot) {
	...
	for(std::vector<std::pair<std::string, std::string> >::const_iterator i = usedInputs.begin(); i != usedInputs.end(); ++i) {
		...
		if(0 <= inputNumber && inputNumber <= 14)
			mask |= (0x0001 << inputNumber);
	}
	return mask;
}
...
void emu::step::Test::configureCrates() {
	...
	for(vector<emu::pc::Crate*>::iterator crate = crates.begin(); crate != crates.end(); ++crate) {
		if((*crate)->IsAlive()) {
			...
			if(pLogger_) {
				LOG4CPLUS_INFO( *pLogger_, "(*crate)->configure( 0 ) in " << ((*crate)->IsAlive()?"live":"dead") << " crate " << (*crate)->GetLabel());
			}
...
//       for ( vector<emu::pc::DAQMB*>::iterator dmb = dmbs.begin(); dmb != dmbs.end(); ++dmb ) printDCFEBUserCodes( *dmb );
			...
			if(pLogger_) {
				LOG4CPLUS_INFO( *pLogger_,
						"(*crate)->ccb()->setCCBMode( emu::pc::CCB::VMEFPGA ) in " << ((*crate)->IsAlive()?"live":"dead") << " crate " << (*crate)->GetLabel());
			}
			(*crate)->ccb()->setCCBMode(emu::pc::CCB::VMEFPGA);
			// Disable all triggers
			if(pLogger_) {
				LOG4CPLUS_INFO( *pLogger_, "(*crate)->ccb()->DisableL1a() in " << ((*crate)->IsAlive()?"live":"dead") << " crate " << (*crate)->GetLabel());
			}
...
void emu::step::Test::disableTrigger() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "Disabling triggers.");
	}
...
void emu::step::Test::setUpDMB(emu::pc::DAQMB *dmb) {
	...
	int pulse_delay = 10;
	int inject_delay = 15;
	int dword = (calibration_LCT_delay & 0xF) | (calibration_l1acc_delay & 0x1F) << 4 | (pulse_delay & 0x1F) << 9 | (inject_delay & 0x1F) << 14;
	...
	if(((dmbstatus[9] & 0xff) == 0x00 || (dmbstatus[9] & 0xff) == 0xff) || ((dmbstatus[8] & 0xff) == 0x00 || (dmbstatus[8] & 0xff) == 0xff)
			|| ((dmbstatus[7] & 0xff) == 0x00 || (dmbstatus[7] & 0xff) == 0xff) || ((dmbstatus[6] & 0xff) == 0x00 || (dmbstatus[6] & 0xff) == 0xff)
...
void emu::step::Test::turnONlvDCFEBandALCT(emu::pc::Crate* crate) {
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "turning all on-chamber boards ON");
	}
	...
	for(vector<emu::pc::DAQMB*>::iterator dmb = dmbs.begin(); dmb != dmbs.end(); ++dmb) {
		...
		if((*dmb)->GetHardwareVersion() == 2) {
			if(pLogger_) {
				LOG4CPLUS_INFO( *pLogger_, "turning on-chamber boards ON for this ODMB");
			}
			int state = (*dmb)->lvmb_power_state();
			if(pLogger_) {
				LOG4CPLUS_INFO( *pLogger_, "low voltage power state before power on: " << state);
			}
			sleep(2);
			...
			if(pLogger_) {
				LOG4CPLUS_INFO( *pLogger_, "low voltage power state after power on: " << state);
			}
...
void emu::step::Test::printDCFEBUserCodes(emu::pc::DAQMB* dmb) {
	...
	for(vector<emu::pc::CFEB>::reverse_iterator cfeb = cfebs.rbegin(); cfeb != cfebs.rend(); ++cfeb) {
		ostringstream oss;
		oss << "CFEB: " << cfeb->number() << ", user code: " << hex << dmb->febfpgauser(*cfeb) << ", Virtex status: " << dmb->virtex6_readreg(7);
		if(pLogger_) {
...
string emu::step::Test::getDataDirName() const {
	...
	for(set<string>::iterator d = dirs.begin(); d != dirs.end(); ++d) {
		struct stat sb;
		if(stat(d->c_str(), &sb) == 0 && (S_ISDIR( sb.st_mode ) || S_ISLNK(sb.st_mode)))
			return *d;
	}
	return string();
}
...
void emu::step::Test::configure_11() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_11 starting");
	}
...
void emu::step::Test::enable_11() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_11 starting");
	}
	progress_.setTotal((int) parameters_["durationInSec"]);
...
void emu::step::Test::configure_12() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_12 (parallel) starting");
	}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_12 (parallel) ending");
	}
}

void emu::step::Test::enable_12() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_12 (parallel) starting");
	}
		...
		for(uint64_t iStrip = 0; iStrip < nStrips; ++iStrip) {
			...
			for(uint64_t iPulse = 1; iPulse <= events_per_strip; ++iPulse) {
				...
				if(iPulse % 100 == 0) {
					if(pLogger_) {
						stringstream ss;
						ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", strip " << iStrip + 1 << "/"
								<< nStrips << ", pulses " << iPulse << "/" << events_per_strip << " (" << progress_.getCurrent() << " of " << nEvents_
								<< " in total)";
						LOG4CPLUS_INFO( *pLogger_, ss.str());
					}
				}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_12 (parallel) ending");
	}
}

void emu::step::Test::configure_13() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_13 (parallel) starting");
	}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_13 (parallel) ending");
	}
}

void emu::step::Test::enable_13() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_13 (parallel) starting");
	}
			...
			for(uint64_t iThreshold = 0; iThreshold < thresholds_per_tpamp; ++iThreshold) {
				...
				for(uint64_t iPulse = 1; iPulse <= events_per_threshold; ++iPulse) {
					...
					if(iPulse % 100 == 0) {
						if(pLogger_) {
							stringstream ss;
							ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", amplitude " << iAmp + 1
									<< "/" << tpamps_per_run << ", threshold " << iThreshold + 1 << "/" << thresholds_per_tpamp << ", pulses " << iPulse << "/"
									<< events_per_threshold << " (" << progress_.getCurrent() << " of " << nEvents_ << " in total)";
							LOG4CPLUS_INFO( *pLogger_, ss.str());
						}
					}
					if(isToStop_)
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_13 (parallel) ending");
	}
}

void emu::step::Test::configure_14() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_14 (parallel) starting");
	}
...
void emu::step::Test::enable_14() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_14 (parallel) starting");
	}
		...
		for(uint64_t iDelay = 0; iDelay < delays_per_run; ++iDelay) {
			...
			for(uint64_t iPulse = 1; iPulse <= events_per_delay; ++iPulse) {
				...
				if(iPulse % 100 == 0) {
					if(pLogger_) {
						stringstream ss;
						ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", delay " << iDelay + 1 << "/"
								<< delays_per_run << ", pulses " << iPulse << "/" << events_per_delay << " (" << progress_.getCurrent() << " of " << nEvents_
								<< " in total)";
						LOG4CPLUS_INFO( *pLogger_, ss.str());
					}
				}
...
void emu::step::Test::configure_15() { // OK
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_15 (parallel) starting");
	}
	...
	for(vector<emu::pc::Crate*>::iterator crate = crates.begin(); crate != crates.end(); ++crate) {
		...
		if(pLogger_) {
			LOG4CPLUS_INFO( *pLogger_, "ext_trig_delay set to " << extTrigDelay);
		}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_15 (parallel) ending");
	}
}

void emu::step::Test::enable_15() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_15 (parallel) starting");
	}
	...
	for(vector<emu::pc::Crate*>::iterator crate = crates.begin(); crate != crates.end(); ++crate) {
		...
		for(uint64_t iTrigger = 1; iTrigger <= events_total; ++iTrigger) {
			...
			if(iTrigger % 100 == 0) {
				if(pLogger_) {
					stringstream ss;
					ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", triggers " << iTrigger << "/"
							<< events_total << " (" << progress_.getCurrent() << " of " << nEvents_ << " in total)";
					LOG4CPLUS_INFO( *pLogger_, ss.str());
				}
			}
...
// 	} // for ( vector<emu::pc::TMB*>::iterator tmb = tmbs.begin(); tmb != tmbs.end(); ++tmb )
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_15 (parallel) ending");
	}
}
		...
		for(uint64_t iLayerPair = 0; iLayerPair < nLayerPairs; ++iLayerPair) { // layer loop
			...
			for(uint64_t iPulse = 1; iPulse <= events_per_layer; ++iPulse) {
				...
				if(iPulse % 100 == 0) {
					if(pLogger_) {
						stringstream ss;
						ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", layer pairs "
								<< iLayerPair + 1 << "/" << nLayerPairs << ", pulses " << iPulse << "/" << events_per_layer << " (" << progress_.getCurrent()
								<< " of " << nEvents_ << " in total)";
						LOG4CPLUS_INFO( *pLogger_, ss.str());
					}
				}
				// 	// Find the time between the DMB's receiving CLCT pretrigger and L1A:
...
void emu::step::Test::configure_17() { // OK
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_17 (parallel) starting");
	}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_17 (parallel) ending");
	}
}

void emu::step::Test::enable_17() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_17 (parallel) starting");
	}
				...
				for(uint64_t iPulse = 1; iPulse <= events_per_delay; ++iPulse) {
					...
					if(iPulse % events_per_delay == 0) {
						if(pLogger_) {
							stringstream ss;
							ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", strip " << iStrip + 1
									<< "/" << strips_per_run << ", delay " << iDelay + 1 << "/" << delays_per_strip << ", pulses " << iPulse << "/"
									<< events_per_delay << " (" << progress_.getCurrent() << " of " << nEvents_ << " in total)";
							LOG4CPLUS_INFO( *pLogger_, ss.str());
						}
					}
					// 	  // Find the time between the DMB's receiving CLCT pretrigger and L1A:
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_17 (parallel) ending");
	}
}

void emu::step::Test::configure_17b() { // OK
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_17b (parallel) starting");
	}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_17b (parallel) ending");
	}
}

void emu::step::Test::enable_17b() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_17b (parallel) starting");
	}
				...
				for(uint64_t iPulse = 1; iPulse <= events_per_pulsedac; ++iPulse) {
					...
					if(iPulse % events_per_pulsedac == 0) {
						if(pLogger_) {
							stringstream ss;
							ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", strip " << iStrip + 1
									<< "/" << strips_per_run << ", DAC setting " << iDACSetting + 1 << "/" << pulse_dac_settings << ", pulses " << iPulse << "/"
									<< events_per_pulsedac << " (" << progress_.getCurrent() << " of " << nEvents_ << " in total)";
							LOG4CPLUS_INFO( *pLogger_, ss.str());
						}
					}
// 	  // Find the time between the DMB's receiving CLCT pretrigger and L1A:
...
// 	  } // for ( vector<emu::pc::TMB*>::iterator tmb = tmbs.begin(); tmb != tmbs.end(); ++tmb )
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_17b (parallel) ending");
	}
}
...
void emu::step::Test::enable_18() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_18 starting");
	}
...
void emu::step::Test::configure_19() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_19 (parallel) starting");
	}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_19 (parallel) ending");
	}
}

void emu::step::Test::enable_19() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_19 (parallel) starting");
	}
					...
					for(uint64_t iPulse = 1; iPulse <= events_per_thresh; ++iPulse) {
						...
						if(iPulse % events_per_thresh == 0) {
							if(pLogger_) {
								stringstream ss;
								ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", strip " << iStrip + 1
										<< "/" << strips_per_run << ", amplitude " << iAmp + 1 << "/" << dmb_tpamps_per_strip << ", threshold "
										<< iThreshold + 1 << "/" << threshs_per_tpamp << ", pulses " << iPulse << "/" << events_per_thresh << " ("
										<< progress_.getCurrent() << " of " << nEvents_ << " in total)";
								LOG4CPLUS_INFO( *pLogger_, ss.str());
							}
						}
						// Find the time between the DMB's receiving CLCT pretrigger and L1A:
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_19 (parallel) ending");
	}
}

void emu::step::Test::configure_21() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_21 (parallel) starting");
	}
	...
	std::cout << "dmb_test_pulse_amp,cfeb_threshold,ext_trig_delay,ext_trig_delay_odmb" << std::endl;
	std::cout << dmb_test_pulse_amp << "," << cfeb_threshold << "," << ext_trig_delay << "," << ext_trig_delay_odmb << std::endl;
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_21 (parallel) ending");
	}
}

void emu::step::Test::enable_21() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_21 (parallel) starting");
	}
	...
	std::cout << "VALUES:" << std::endl;
	std::cout << "events_per_hstrip,hstrips_per_run,hstrip_first,hstrip_step,msec_between_pulses" << std::endl;
	std::cout << events_per_hstrip << "," << hstrips_per_run << "," << hstrip_first << "," << hstrip_step << "," << msec_between_pulses << std::endl;
	...
	for(vector<emu::pc::Crate*>::iterator crate = crates.begin(); crate != crates.end(); ++crate) {
		...
		std::cout << "VALUES_RB:" << std::endl;
		std::cout << "tmb_l1a_delay,ext_trig_delay" << std::endl;
		(*crate)->tmbs()[0]->ReadRegister(0x00002C);
		std::cout << (*crate)->tmbs()[0]->GetInternalL1aDelay() << "," << (((*crate)->ccb()->ReadRegister(emu::pc::CCB::CSRB5) >> 8) & 0xff) << std::endl;
		...
		for(uint64_t iHalfStrip = 0; iHalfStrip < hstrips_per_run; ++iHalfStrip) {
			...
			for(uint64_t iPulse = 1; iPulse <= events_per_hstrip; ++iPulse) {
				...
				for(vector<emu::pc::TMB *>::iterator tmb = tmbs.begin(); tmb != tmbs.end(); ++tmb) {
					(*tmb)->GetCounters();
					std::cout << "####" << i_tmb << "," << iHalfStrip << "," << std::dec << (*tmb)->GetCounter(14);
					for(int i=55; i<=60; ++i) {
						std::cout << "," << std::dec << (*tmb)->GetCounter(i);
						usleep(1);
					}
					std::cout << std::endl;
					++i_tmb;
				...
				*/
				...
				if(iPulse % 100 == 0) {
					if(pLogger_) {
						stringstream ss;
						ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", half strip " << iHalfStrip + 1
								<< "/" << hstrips_per_run << " (" << halfStrip << ")" << ", pulses " << iPulse << "/" << events_per_hstrip << " ("
								<< progress_.getCurrent() << " of " << nEvents_ << " in total)";
						LOG4CPLUS_INFO( *pLogger_, ss.str());
					}
				}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_21 (parallel) ending");
	}
}
...
void emu::step::Test::configure_25() {
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_25 (multichamber) starting");
	}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_25 (multichamber) ending");
	}
}
...
void emu::step::Test::enable_25() {
	...
	// In principle, it could be run in parallel in different crates, but only if their data all go into different files...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_25 (multichamber) starting");
	}
	...
	if(dataDir.size() == 0) {
		if(pLogger_) {
			LOG4CPLUS_WARN( *pLogger_, "No data directory found on this host. Time stamp files for Test 25  will be saved in /tmp instead.");
		}
		dataDir = "/tmp";
	}
	...
	for(vector<emu::pc::Crate*>::iterator crate = crates.begin(); crate != crates.end(); ++crate) {
		...
		for(vector<emu::pc::TMB*>::iterator tmb = tmbs.begin(); tmb != tmbs.end(); ++tmb) {
			...
			timeStampFileName << "Test25" << "_CrateId" << setfill('0') << setw(2) << (*crate)->CrateID() << "_TMBslot" << setfill('0') << setw(2)
					<< (*tmb)->slot() << "_" << setfill('0') << setw(8) << runNumber_ << "_" << runStartTime_ << ".txt";
			ofstream timeStampFile;
			timeStampFile.open((dataDir + "/" + timeStampFileName.str()).c_str());
			if(pLogger_) {
				LOG4CPLUS_INFO( *pLogger_, "Opened " << dataDir << "/" << timeStampFileName.str());
			}
			...
			for(uint64_t iTriggerSetting = 0; iTriggerSetting < trig_settings; ++iTriggerSetting) {
				...
				timeStampFile << (end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec) / 1000. << "\t\t" << l1a_counter << endl;

				if(pLogger_) {
					stringstream ss;
					ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", TMB " << tmb - tmbs.begin() + 1
							<< "/" << tmbs.size() << " in slot " << (*tmb)->slot() << ", trigger setting " << iTriggerSetting + 1 << "/" << trig_settings
							<< ", L1A " << l1a_counter;
					LOG4CPLUS_INFO( *pLogger_, ss.str());
				}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_25 (multichamber) ending");
	}
}

void emu::step::Test::configure_27() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_27 starting");
	}
...
void emu::step::Test::enable_27() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_27 starting");
	}
...
void emu::step::Test::configure_30() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_30 (multichamber) starting");
	}
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_30 (multichamber) ending");
	}
}

void emu::step::Test::enable_30() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_30 (multichamber) starting");
	}
	...
	for(vector<emu::pc::Crate*>::iterator crate = crates.begin(); crate != crates.end(); ++crate) {
		...
		stringstream timeStampFileName;
		timeStampFileName << "Test30_" << (*crate)->GetLabel() << "_PipelineDepth_" << fromDepth << "_to_" << toDepth << "_" << dateTime << ".txt";
		ofstream timeStampFile;
		...
		for(uint64_t iDepth = fromDepth; iDepth <= toDepth; ++iDepth) {
			...
			timeStampFile << "Pipeline_Depth_" << iDepth << "\t\t" << (end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec) / 1000. << "\t\t"
					<< l1a_counter << endl;

			if(pLogger_) {
				stringstream ss;
				ss << "Crate " << (*crate)->GetLabel() << " " << crate - crates.begin() + 1 << "/" << crates.size() << ", Pipeline_Depth " << iDepth << ", L1A "
						<< l1a_counter;
				LOG4CPLUS_INFO( *pLogger_, ss.str());
			}
		}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_30 (multichamber) ending");
	}
}

void emu::step::Test::configure_fake() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_fake starting");
	}
	...
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::configure_fake ending");
	}
}

void emu::step::Test::enable_fake() {
	if(pLogger_) {
		LOG4CPLUS_INFO( *pLogger_, "emu::step::Test::enable_fake starting");
	}
	...
	for(uint64_t iEvent = 0; iEvent < nEvents; ++iEvent) {
		...
		if(pLogger_) {
			LOG4CPLUS_INFO( *pLogger_, "Event " << iEvent + 1);
		}
...
void ( emu::step::Test::* emu::step::Test::getProcedure(const string& testId, State_t state) )() {
			if(isFake_)
				return (state == forConfigure_?&emu::step::Test::configure_fake:&emu::step::Test::enable_fake);
			if(testId == "11")
				return (state == forConfigure_?&emu::step::Test::configure_11:&emu::step::Test::enable_11);
			if(testId == "12")
				return (state == forConfigure_?&emu::step::Test::configure_12:&emu::step::Test::enable_12);
			if(testId == "13")
				return (state == forConfigure_?&emu::step::Test::configure_13:&emu::step::Test::enable_13);
			if(testId == "14")
				return (state == forConfigure_?&emu::step::Test::configure_14:&emu::step::Test::enable_14);
			if(testId == "15")
				return (state == forConfigure_?&emu::step::Test::configure_15:&emu::step::Test::enable_15);
			if(testId == "16")
				return (state == forConfigure_?&emu::step::Test::configure_16:&emu::step::Test::enable_16);
			if(testId == "17")
				return (state == forConfigure_?&emu::step::Test::configure_17:&emu::step::Test::enable_17);
			if(testId == "17b")
				return (state == forConfigure_?&emu::step::Test::configure_17b:&emu::step::Test::enable_17b);
			if(testId == "18")
				return (state == forConfigure_?&emu::step::Test::configure_18:&emu::step::Test::enable_18);
			if(testId == "19")
				return (state == forConfigure_?&emu::step::Test::configure_19:&emu::step::Test::enable_19);
			if(testId == "21")
				return (state == forConfigure_?&emu::step::Test::configure_21:&emu::step::Test::enable_21);
			if(testId == "25")
				return (state == forConfigure_?&emu::step::Test::configure_25:&emu::step::Test::enable_25);
			if(testId == "27")
				return (state == forConfigure_?&emu::step::Test::configure_27:&emu::step::Test::enable_27);
			if(testId == "30")
				return (state == forConfigure_?&emu::step::Test::configure_30:&emu::step::Test::enable_30);
			return NULL;
		}
