...
namespace rubuilder { namespace utils { // namespace rubuilder::utils
  ...
  public:
    ...
    virtual void appendConfigurationItems(utils::InfoSpaceItems&) {};
  ...
  private:
    ...
    #else
    mutable boost::shared_mutex eventMutex_;
    #endif
...
)
{
  ...
  #else
  boost::unique_lock<boost::shared_mutex> uniqueLock(eventMutex_);
  #endif
...
)
{
  ...
  boost::mutex::scoped_lock sl(eventMutex_);
  #else
  boost::shared_lock<boost::shared_mutex> sharedLock(eventMutex_);
  #endif
