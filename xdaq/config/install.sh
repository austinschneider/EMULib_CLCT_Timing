#!/bin/bash
#
# install.sh <source> <destination> <permission> RPM_BUILD_ROOT PACKAGEDIR PACKAGENAME VERSION
#
SRC=$1
DST=$2
PERM=$3
...
if [ -h "$SRC" ]; then
  ...
  if [ -f "$SRC" ]; then
    ...
    MIME=`file -ib $SRC`
    if [[ "$MIME" =~ 'application/(x-executable|x-sharedlib).*' ]]; then
      ...
      #objcopy --add-gnu-debuglink=$RPM_BUILD_ROOT/usr/lib/debug${DST}.debug $RPM_BUILD_ROOT/${DST}
    fi
  fi
fi
