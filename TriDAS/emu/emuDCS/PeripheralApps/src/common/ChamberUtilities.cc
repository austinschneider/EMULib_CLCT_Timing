...
void ChamberUtilities::CFEBTiming_Configure(int * tof) {
	...
	thisDMB_->RedirectOutput(&std::cout);
	thisCCB_->RedirectOutput(&std::cout);
	thisMPC_->RedirectOutput(&std::cout);
	//
	...
	//
	thisTMB_->ReadRegister(vme_ddd1_adr); // Get phase and TOF delay from hardware
	thisTMB_->ReadRegister(vme_ddd2_adr); //
	int initial_cfeb_tof_delay[5] = {thisTMB_->GetReadCfeb0TOFDelay(),
			thisTMB_->GetReadCfeb1TOFDelay(),
			thisTMB_->GetReadCfeb2TOFDelay(),
			thisTMB_->GetReadCfeb3TOFDelay(),
			thisTMB_->GetReadCfeb4TOFDelay()};
	//
	thisTMB_->ReadRegister(phaser_cfeb0_rxd_adr); // Get phaser information
	thisTMB_->ReadRegister(phaser_cfeb1_rxd_adr); //
	thisTMB_->ReadRegister(phaser_cfeb2_rxd_adr); //
	thisTMB_->ReadRegister(phaser_cfeb3_rxd_adr); //
	thisTMB_->ReadRegister(phaser_cfeb4_rxd_adr); //
	int initial_cfeb_phase[5] = { thisTMB_->GetReadCfeb0RxClockDelay(),
			thisTMB_->GetReadCfeb1RxClockDelay(),
			thisTMB_->GetReadCfeb2RxClockDelay(),
			thisTMB_->GetReadCfeb3RxClockDelay(),
			thisTMB_->GetReadCfeb4RxClockDelay() };
	int initial_cfeb_posneg[5]= { thisTMB_->GetReadCfeb0RxPosNeg(),
			thisTMB_->GetReadCfeb1RxPosNeg(),
			thisTMB_->GetReadCfeb2RxPosNeg(),
			thisTMB_->GetReadCfeb3RxPosNeg(),
			thisTMB_->GetReadCfeb4RxPosNeg() };
	//
	thisTMB_->ReadRegister(seq_trig_en_adr);
	int initial_clct_pretrig_enable           = thisTMB_->GetReadClctPatternTrigEnable();    //0x68
	//
	thisTMB_->ReadRegister(tmb_trig_adr);
	int initial_clct_trig_enable              = thisTMB_->GetReadTmbAllowClct();             //0x86
	//
	thisTMB_->ReadRegister(seq_clct_adr);
	int initial_clct_halfstrip_pretrig_thresh = thisTMB_->GetReadHsPretrigThresh();          //0x70
	int initial_clct_pattern_thresh           = thisTMB_->GetReadMinHitsPattern();           //0x70
	//
	thisTMB_->ReadRegister(layer_trg_mode_adr);
	int initial_layer_trig_enable             = thisTMB_->GetReadEnableLayerTrigger();       //0xf0
	//
	thisTMB_->ReadRegister(ccb_trig_adr);
	int initial_ignore_ccb_startstop          = thisTMB_->GetReadIgnoreCcbStartStop();       //0x2c
	//
	// Set up for this test...
	thisTMB_->SetClctPatternTrigEnable(1);
	thisTMB_->SetClctExtTrigEnable(1);
	thisTMB_->WriteRegister(seq_trig_en_adr);
	//
	thisTMB_->SetTmbAllowClct(1);
	thisTMB_->WriteRegister(tmb_trig_adr);
	//
	thisTMB_->SetHsPretrigThresh(5);
	//thisTMB_->SetMinHitsPattern(5); // 5 is used for actual patterns
	thisTMB_->SetMinHitsPattern(1); // 1 used for simple test
	...
	if(tof == NULL) {
		...
		thisTMB_->SetCfeb0TOFDelay(tof[0]);
		thisTMB_->SetCfeb1TOFDelay(tof[1]);
		thisTMB_->SetCfeb2TOFDelay(tof[2]);
		thisTMB_->SetCfeb3TOFDelay(tof[3]);
		thisTMB_->SetCfeb4TOFDelay(tof[4]);
	}
...
/*
int ChamberUtilities::GetOutputHalfStrip(int cfeb, int input_halfstrip, bool is_halfstrip) {
	const int HS_per_CFEB = (is_halfstrip)?32:16;
...
int ChamberUtilities::GetTMBL1ADelay() {
	thisTMB_->ReadRegister(seq_l1a_adr);
	//return thisTMB_->GetInternalL1aDelay();
	return thisTMB_->GetL1aDelay();
}
...
int ChamberUtilities::GetTMBInternalL1ADelay() {
	thisTMB_->ReadRegister(seq_l1a_adr);
	//return thisTMB_->GetInternalL1aDelay();
	return thisTMB_->GetInternalL1a();
}
...
void ChamberUtilities::Step_21() {
	configureCrate();
	configure_21();
	enable_21();
}
void ChamberUtilities::configureCrate() {
	thisDMB_->getCrate()->configure(0);
	::sleep(1);
	thisCCB_->HardReset_crate();
	::sleep(1);
	thisTMB_->tmb_hard_reset_tmb_fpga();
	thisCCB_->setCCBMode(emu::pc::CCB::VMEFPGA);
	thisCCB_->DisableL1a();
}
void ChamberUtilities::configure_21() {
	//DOUBLE CHECK CONSTANTS WITH DMB Debug
	uint64_t dmb_test_pulse_amp = 160;
	uint64_t cfeb_threshold = 50;
	...
	uint64_t ext_trig_delay = 17;
	//uint64_t ext_trig_delay_odmb = 23;
	uint64_t ext_trig_delay_odmb = 0;
	//uint64_t tmb_l1a_delay = 84;
	uint64_t tmb_l1a_delay = 103;

	SetDCFEBsPipelineDepth(58);

	thisTMB_->tmb_hard_reset_tmb_fpga();
	CFEBTiming_ConfigureODMB();

	usleep(1000); //MISSING from DMB Debug
	thisCCB_->l1aReset();
	usleep(1000); //MISSING from DMB Debug
	ResyncDCFEBs(); // TODO: remove once firmware takes care of it

	thisCCB_->EnableL1aFromDmbCfebCalibX();

	// Take the hardware version of the first DMB, assuming ODMB and copper DMB are never tested simulatenously!!!
	uint64_t extTrigDelay = (thisDMB_->GetHardwareVersion() == 2?ext_trig_delay_odmb:ext_trig_delay); //MISSING from DMB Debug
	thisCCB_->SetExtTrigDelay(extTrigDelay); // Delay of ALCT and CLCT external triggers before distribution to backplane //MISSING from DMB Debug

	SetODMBPedestalMode();

	thisDMB_->set_dac((float) dmb_test_pulse_amp * 5. / 4095., 0); // set inject amplitude - first parameter (same for the entire test)
	thisDMB_->set_comp_thresh((float) cfeb_threshold / 1000.); // set cfeb thresholds (for the entire test)

	if(thisDMB_->GetHardwareVersion() < 2)
		thisDMB_->settrgsrc(0); // disable DMB's own trigger, LCT

	thisTMB_->EnableClctExtTrig(); //MISSING from DMB Debug

	SetTMBL1ADelay(tmb_l1a_delay);
}
void ChamberUtilities::enable_21() {
	std::ostream noBuffer(NULL);
	uint64_t events_per_hstrip = 250;
	uint64_t hstrips_per_run = 32;
	uint64_t hstrip_first = 1;
	uint64_t hstrip_step = 1;
	...
	for(uint64_t iHalfStrip = 0; iHalfStrip < hstrips_per_run; ++iHalfStrip) {
		...
		thisDMB_->trighalfx(halfStrip);

		if(thisDMB_->GetHardwareVersion() == 2) { // All CFEBs should have the same HW version; get it from the first.
			usleep(100000); // buck shifting takes a lot more time for DCFEBs (should check this)
			thisCCB_->bc0(); // may not need this (should check)
			usleep(100000);
		}

		thisCCB_->RedirectOutput(&noBuffer); // ccb prints a line on each test pulse - waste it

		std::cout << "VALUES:" << std::endl;
		std::cout << "tmb_l1a_delay,ext_trig_delay" << std::endl;
		std::cout << GetTMBL1ADelay() << "," << GetExtTrigDelay() << std::endl;

		for(uint64_t iPulse = 1; iPulse <= events_per_hstrip; ++iPulse) {
			thisCCB_->GenerateDmbCfebCalib1(); // pulse
			usleep(10 + 1000 * msec_between_pulses);
			// CFEBTiming Code
			int i_tmb = 0;
			thisTMB_->GetCounters();
			std::cout << "####" << i_tmb << "," << iHalfStrip << "," << std::dec << thisTMB_->GetCounter(14);
			for(int i=55; i<=60; ++i) {
				std::cout << "," << std::dec << thisTMB_->GetCounter(i);
				usleep(1);
			}
			std::cout << std::endl;
			...
			std::cout << "%%%%";
			for(int i=0; i<60; ++i) {
				std::cout << "," << std::dec << thisDMB_->ReadRegister((DMBCounters[i] << 4) | 0x300C);
			}
			std::cout << std::endl;

			// CFEBTiming Code

			thisCCB_->RedirectOutput(&std::cout); // get back ccb output
...
void ChamberUtilities::CFEBTiming_DMBDebug(bool print_data, bool print_clct) {


	(*MyOutput_) << "Start CFEBTiming_DMBDebug" << std::endl;

	if(false) {
		int * l1a_delays = CFEBTiming_L1AWindowScan();
		(*MyOutput_) << "After CFEBTiming_L1AWindowScan" << std::endl;
		if(l1a_delays == NULL) {
			(*MyOutput_) << "Error! NULL delay pointer!" << std::endl;
			return;
		}
	}

	std::ostream * screen_output = MyOutput_;
	std::fstream out_file("/home/cscme11/aschneid/me11/DMB_Debug_Output.txt", std::ios_base::out);
	std::fstream packet_file("/home/cscme11/aschneid/me11/DMB_Debug_Packets", std::ios_base::out);
	std::fstream clct_file("/home/cscme11/aschneid/me11/DMB_Debug_CLCTs", std::ios_base::out);
	MyOutput_ = &out_file;
	...
	int bad_rx_delay[2][MaxTimeDelay]; memset(bad_rx_delay, 0, sizeof(bad_rx_delay));

	out_file << std::setw(4) << " ";
	for(int rx_delay = 0; rx_delay<MaxTimeDelay; ++rx_delay) {
		out_file << std::setw(4) << rx_delay;
	}
	out_file << std::endl;

	int random_ihs_list[MaxHalfStrip];
	for(int i=0; i<MaxHalfStrip; ++i) {
		random_ihs_list[i] = i;
	}
	std::random_shuffle(random_ihs_list, random_ihs_list + MaxHalfStrip);

	int random_pattern_list[MaxPattern - 1];
	for(int i=2; i<=MaxPattern; ++i) {
		random_pattern_list[i] = i;
	}
	std::random_shuffle(random_pattern_list, random_pattern_list + (MaxPattern - 1));
	...
	//SetCfebRxPosNeg(posneg); // Set posneg
		...
		for(int rx_delay = 0; rx_delay<MaxTimeDelay; ++rx_delay) {
			...
			packet_file << " dc" << std::hex << std::setw(2) << std::setfill('0') << ((posneg << 5) | (rx_delay));
			...
			for(int i = 0; i<5; ++i) {
				for(int halfstrip = 0; halfstrip<MaxHalfStrip; ++halfstrip) {
					...
					if(print_data) {
						...
						if(n_words != 0)
							(*screen_output) << "ERROR! n_words != 0" << std::endl;
					}

					CFEBTiming_CheckConfiguration(config);
					usleep(1000);

					CFEBTiming_PulseInject(is_inject_scan, cfeb, AllLayers, pattern, halfstrip); // Inject a pattern
				...
				//(*MyOutput_) << std::setw(4) << bad_rx_delay[posneg][rx_delay];
			}
			//(*MyOutput_) << std::endl;
			//(*MyOutput_) << std::endl << std::endl;
		}
	}
	MyOutput_ = screen_output;
}
...
void ChamberUtilities::Print_ODMB_FIFO() {
	int n_words = thisDMB_->ReadRegister(0x550C);
	if(n_words > 1372) n_words = thisDMB_->ReadRegister(0x550C);
	...
	for(int i=0; i<n_words; ++i) {
		words[i] = thisDMB_->ReadRegister(0x5500);
...
void ChamberUtilities::Print_CLCT0() {
	(*MyOutput_) << "----------------------"                                   << std::endl;
	(*MyOutput_) << "CLCT0.Valid      = 0x" << std::hex << thisTMB_->GetCLCT0Valid()  							<< std::endl;
	(*MyOutput_) << "CLCT0.Nhits      = 0x" << std::hex << thisTMB_->GetCLCT0Nhit()         				<< std::endl;
	(*MyOutput_) << "CLCT0.pattern    = 0x" << std::hex << thisTMB_->GetCLCT0PatternId()      			<< std::endl;
	(*MyOutput_) << "CLCT0.Key HStrip = "   << std::dec << thisTMB_->GetCLCT0keyHalfStrip() 				<< std::endl;
	(*MyOutput_) << "CLCT0.sync err   = 0x" << std::hex << thisTMB_->GetCLCTBX0SyncErrEnable()      << std::endl;
	//
	(*MyOutput_) << std::endl;
}
...
void ChamberUtilities::Print_CFEB_Masks() {
	thisTMB_->ReadRegister(seq_trig_en_adr);
	thisTMB_->ReadRegister(cfeb_inj_adr);
	thisTMB_->GetReadEnableCLCTInject();


}

int * ChamberUtilities::CFEBTiming_L1AWindowScan(bool print_data, bool print_clct) {
	std::fstream out_file("/home/cscme11/aschneid/me11/L1AWindowScan_Output.txt", std::ios_base::out);
	std::fstream packet_file("/home/cscme11/aschneid/me11/L1AWindowScan_Packets", std::ios_base::out);
	std::fstream clct_file("/home/cscme11/aschneid/me11/L1AWindowScan_CLCTs", std::ios_base::out);
	...
	CFEBTiming_Configuration test_config;
	test_config.cfeb_pipeline_depth = -999;
	test_config.dac = -999;
	test_config.comp_thresh = -999;
	test_config.clct_pattern_trig_en = -999;
	test_config.clct_ext_trig_en = -999;
	test_config.tmb_allow_clct = -999;
	test_config.hs_pretrig_hit_thresh = -999;
	test_config.min_hits_pattern = -999;
	test_config.ignore_ccb_startstop = -999;
	test_config.layer_trigger_en = -999;
	test_config.fifo_mode = -999;
	test_config.fifo_tbins = -999;
	test_config.fifo_pretrig = -999;
	test_config.fifo_no_hits_raw = -999;
	test_config.tmb_internal_l1a = -999;
	test_config.ccb_ext_trig_delay = -999;
	test_config.tmb_l1a_delay = -999;
	test_config.cfeb_rx_posneg = -999;
	test_config.cfeb_rx_clock_delay = -999;

	thisCCB_->HardReset_crate();

	CFEBTiming_CheckConfiguration(test_config); // Print out initial configuration
	...
	while(!has_bad && i<1000000) {
		CFEBTiming_ConfigureLevel(config, 2);
		CFEBTiming_ReadConfiguration(read);
		if(read.hs_pretrig_hit_thresh != config.hs_pretrig_hit_thresh) {
			has_bad = true;
			std::cout << i+1 << " until bad" << std::endl;
			while(!has_good && j<1000000) {
				CFEBTiming_ReadConfiguration(read);
				if(read.hs_pretrig_hit_thresh == config.hs_pretrig_hit_thresh) {
					has_good = true;
					std::cout << j+1 << "until good" << std::endl;
				}
				++j;
			}
			if(!has_good) {
				std::cout << "Not good after 1E6 reads, trying write/read" << std::endl;
				j = 0;
				while(!has_good && j<1000000) {
					CFEBTiming_ConfigureLevel(config, 2);
					CFEBTiming_ReadConfiguration(read);
	...
	*/
	...
	for(int window = 0; window<n_windows; ++window) {
		...
		config.ccb_ext_trig_delay = e_min;
		...
		for(int i = i_min; i<=i_max; ++i) {
			...
			if(pulses==n_pulses) {
				...
				thisTMB_->PrintCounters(55);
				thisTMB_->PrintCounters(56);
				thisTMB_->PrintCounters(57);
				thisTMB_->PrintCounters(58);
				if(in_window)
					in_window = false;
			}
	...
	if(window_index >= 0) {
		...
		this->ccb_l1a_delay = ret[0];
		this->tmb_l1a_delay = ret[1];
	}
	else {
...
void ChamberUtilities::CFEBTiming_HalfStripScan() {
	const int MaxTimeDelay=25;
	const int MaxLayer = 6;
	const int LayerCombinations = 64;
	const int MaxHalfStrip = 32;
	...
	// Count of how many times each half-strip appears for each time delay
	int hs_count[MaxPosNeg][MaxHalfStrip][HalfStripCombinations]; memset(hs_count, 0, sizeof(hs_count));
	int hits_count[MaxPosNeg][MaxHalfStrip][LayerCombinations]; memset(hits_count, 0, sizeof(hits_count));
	int pattern_count[MaxPosNeg][MaxHalfStrip][PatternCombinations]; memset(pattern_count, 0, sizeof(pattern_count));

	int random_ihs_list[NTimes*MaxHalfStrip];
	for(int i=0; i<NTimes*MaxHalfStrip; ++i) {
		random_ihs_list[i] = i%MaxHalfStrip;
	}
	std::random_shuffle(random_ihs_list, random_ihs_list + NTimes*MaxHalfStrip);

	CFEBTiming_Configure();

	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		SetCfebRxPosNeg(posneg);
		SetCfebRxClockDelay(delay);
		for(int i = 0; i<NTimes*MaxHalfStrip; ++i) {
			int ihs = random_ihs_list[i];
			CFEBTiming_PulseInject(is_inject_scan, cfeb, AllLayers, MaxPattern, ihs);
			thisTMB_->DecodeCLCT();
			// Check bounds on values when accessing arrays!!!!
			hs_count[posneg][ihs][thisTMB_->GetCLCT0keyHalfStrip()]++;
			hits_count[posneg][ihs][thisTMB_->GetCLCT0Nhit()]++;
			pattern_count[posneg][ihs][thisTMB_->GetCLCT0PatternId()]++;
		}
	}

	(*MyOutput_) << "#### Half-strip test ####" << std::endl;
	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		for(int ihs = 0; ihs<MaxHalfStrip; ++ihs) {
			(*MyOutput_) << "Half-Strip: " << std::string((posneg)?("+"):("-")) << std::dec << std::setw(2) << ihs << std::endl;
			for(int halfstrip = 0; halfstrip<HalfStripCombinations; ++halfstrip) {
				if(hs_count[posneg][ihs][halfstrip] > 0) {
					(*MyOutput_) << "    " << std::setw(2) << halfstrip % MaxHalfStrip << ": " << hs_count[posneg][ihs][halfstrip] << std::endl;
				}
			}
		}
	}
	...
	(*MyOutput_) << "#### Hits test ####" << std::endl;
	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		for(int ihs = 0; ihs<MaxHalfStrip; ++ihs) {
			(*MyOutput_) << "Half-Strip: " << std::string((posneg)?("+"):("-")) << std::dec << std::setw(2) << ihs << std::endl;
			for(int hits = 0; hits<LayerCombinations; ++hits) {
				if(hits_count[posneg][ihs][hits] > 0) {
					(*MyOutput_) << "    " << std::setw(2) << hits << ": " << hits_count[posneg][ihs][hits] << std::endl;
				}
			}
		}
	}
	...
	(*MyOutput_) << "#### Pattern test ####" << std::endl;
	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		for(int ihs = 0; ihs<MaxHalfStrip; ++ihs) {
			(*MyOutput_) << "Half-Strip: " << std::string((posneg)?("+"):("-")) << std::dec << std::setw(2) << ihs << std::endl;
			for(int pattern = 0; pattern<PatternCombinations; ++pattern) {
				if(pattern_count[posneg][ihs][pattern] > 0) {
					(*MyOutput_) << "    " << std::hex << std::setw(2) << pattern << ": " << std::dec << pattern_count[posneg][ihs][pattern] << std::endl;
				}
			}
		}
	}
...
void ChamberUtilities::CFEBTiming_LayerMaskScan() {
	int * l1a_delays = CFEBTiming_L1AWindowScan();
	if(l1a_delays == NULL) {
		(*MyOutput_) << "Error! NULL delay pointer!" << std::endl;
		return;
	...
	std::fstream out_file("/home/cscme11/aschneid/me11/LayerMaskScan_Output.txt", std::ios_base::out);
	std::fstream packet_file("/home/cscme11/aschneid/me11/packets", std::ios_base::out);
	out_file << "BEGIN" << std::endl << std::flush;
	std::ostream * temp_os = MyOutput_;
	MyOutput_ = &out_file;
	...
	CFEBTiming_Configuration config;
	config.cfeb_pipeline_depth = 58;
	config.dac = (float) 160 * 5. / 4095.;
	config.comp_thresh = (float) 50 / 1000.;

	config.clct_ext_trig_en = 1;
	config.ccb_ext_trig_delay = l1a_delays[0];
	config.tmb_l1a_delay = l1a_delays[1];
	config.cfeb_rx_posneg = 0;
	config.cfeb_rx_clock_delay = 0;

	int halfstrip_mapping[32] = {127, 126, 125, 124, 123, 122, 121, 120, 119, 118, 117, 116, 115, 114, 113, 112, 111, 110, 109, 108, 107, 106, 105, 104, 103, 102, 101, 100, 99, 98, 97, 96};

	int bad_rx_delay[2][MaxTimeDelay]; memset(bad_rx_delay, 0, sizeof(bad_rx_delay));

	(*MyOutput_) << std::setw(4) << " ";
	for(int rx_delay = 0; rx_delay<MaxTimeDelay; ++rx_delay) {
		(*MyOutput_) << std::setw(4) << rx_delay;
	}
	(*MyOutput_) << std::endl;
	...
	for(int layers = 0; layers<=AllLayers; ++layers) {
		...
		for(int halfstrip = 0; halfstrip<MaxHalfStrip; ++halfstrip) {
			...
			CFEBTiming_PulseInject(is_inject_scan, cfeb, AllLayers, pattern, random_ihs_list[halfstrip]); // Inject a pattern
...
void ChamberUtilities::CFEBTiming_LayerMaskScan() {
	...
	int halfstrip = last_halfstrip;

	const int delay = 10;

	// Count of how many times each half-strip appears for each time delay
	int hs_count[MaxPosNeg][AllLayers+1][HalfStripCombinations]; memset(hs_count, 0, sizeof(hs_count));
	int hits_count[MaxPosNeg][AllLayers+1][LayerCombinations]; memset(hits_count, 0, sizeof(hits_count));
	int pattern_count[MaxPosNeg][AllLayers+1][PatternCombinations]; memset(pattern_count, 0, sizeof(pattern_count));
	...
	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		...
		for(int layers = 0; layers<=AllLayers; ++layers) {
			...
			for(int i=0; i<NTimes; ++i) {
				while(halfstrip==last_halfstrip) {
					halfstrip = rand() % MaxHalfStrip;
				}
				CFEBTiming_PulseInject(is_inject_scan, cfeb, layers, MaxPattern, halfstrip);
				thisTMB_->DecodeCLCT();
				// Check bounds on values when accessing arrays!!!!
				hs_count[posneg][layers][thisTMB_->GetCLCT0keyHalfStrip()]++;
				hits_count[posneg][layers][thisTMB_->GetCLCT0Nhit()]++;
				pattern_count[posneg][layers][thisTMB_->GetCLCT0PatternId()]++;
			}
		}
	}
	...
	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		for(int layers = 0; layers<=AllLayers; ++layers) {
			(*MyOutput_) << "Layer: " << std::string((posneg)?("+"):("-")) << std::dec << std::setw(2) << layers << std::endl;
			for(int halfstrip = 0; halfstrip<HalfStripCombinations; ++halfstrip) {
				if(hs_count[posneg][layers][halfstrip] > 0) {
					(*MyOutput_) << "    " << std::setw(2) << halfstrip % MaxHalfStrip << ": " << hs_count[posneg][layers][halfstrip] << std::endl;
				}
			}
		}
	}
	...
	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		for(int layers = 0; layers<=AllLayers; ++layers) {
			(*MyOutput_) << "Layer: " << std::string((posneg)?("+"):("-")) << std::dec << std::setw(2) << layers << std::endl;
			for(int hits = 0; hits<LayerCombinations; ++hits) {
				if(hits_count[posneg][layers][hits] > 0) {
					(*MyOutput_) << "    " << std::setw(2) << hits << ": " << hits_count[posneg][layers][hits] << std::endl;
				}
			}
		}
	}
	...
	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		for(int layers = 0; layers<=AllLayers; ++layers) {
			(*MyOutput_) << "Layer: " << std::string((posneg)?("+"):("-")) << std::dec << std::setw(2) << layers << std::endl;
			for(int pattern = 0; pattern<PatternCombinations; ++pattern) {
				if(pattern_count[posneg][layers][pattern] > 0) {
					(*MyOutput_) << "    " << std::hex << std::setw(2) << pattern << ": " << std::dec << pattern_count[posneg][layers][pattern] << std::endl;
				}
			}
		}
	}
...
void ChamberUtilities::CFEBTiming_Debug() {
	const int MaxTimeDelay=25;
	const int MaxLayer = 6;
	const int LayerCombinations = 64;
	const int MaxHalfStrip = 32;
	...
	// Count of how many times each half-strip appears for each time delay
	int hs_count[MaxPosNeg][MaxTimeDelay][HalfStripCombinations]; memset(hs_count, 0, sizeof(hs_count));
	int hits_count[MaxPosNeg][MaxTimeDelay][LayerCombinations]; memset(hits_count, 0, sizeof(hits_count));
	int pattern_count[MaxPosNeg][MaxTimeDelay][PatternCombinations]; memset(pattern_count, 0, sizeof(pattern_count));

	CFEBTiming_Configure();

	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		SetCfebRxPosNeg(posneg);
		for(int delay = 0; delay<MaxTimeDelay; ++delay) {
			SetCfebRxClockDelay(delay);
			for(int i=0; i<NTimes; ++i) {
				CFEBTiming_PulseInject(is_inject_scan, cfeb, AllLayers, MaxPattern, halfstrip);
				thisTMB_->DecodeCLCT();
				// Check bounds on values when accessing arrays!!!!
				hs_count[posneg][delay][thisTMB_->GetCLCT0keyHalfStrip()]++;
				hits_count[posneg][delay][thisTMB_->GetCLCT0Nhit()]++;
				pattern_count[posneg][delay][thisTMB_->GetCLCT0PatternId()]++;
			}
		}
	}
	...
	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		for(int delay = 0; delay<MaxTimeDelay; ++delay) {
			(*MyOutput_) << "Time: " << std::string((posneg)?("+"):("-")) << std::dec << std::setw(2) << delay << std::endl;
			for(int halfstrip = 0; halfstrip<HalfStripCombinations; ++halfstrip) {
				if(hs_count[posneg][delay][halfstrip] > 0) {
					(*MyOutput_) << "    " << std::setw(2) << halfstrip % MaxHalfStrip << ": " << hs_count[posneg][delay][halfstrip] << std::endl;
				}
			}
		}
	}
	...
	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		for(int delay = 0; delay<MaxTimeDelay; ++delay) {
			(*MyOutput_) << "Time: " << std::string((posneg)?("+"):("-")) << std::dec << std::setw(2) << delay << std::endl;
			for(int hits = 0; hits<LayerCombinations; ++hits) {
				if(hits_count[posneg][delay][hits] > 0) {
					(*MyOutput_) << "    " << std::setw(2) << hits << ": " << hits_count[posneg][delay][hits] << std::endl;
				}
			}
		}
	}
	...
	for(int posneg = 0; posneg<MaxPosNeg; ++posneg) {
		for(int delay = 0; delay<MaxTimeDelay; ++delay) {
			(*MyOutput_) << "Time: " << std::string((posneg)?("+"):("-")) << std::dec << std::setw(2) << delay << std::endl;
			for(int pattern = 0; pattern<PatternCombinations; ++pattern) {
				if(pattern_count[posneg][delay][pattern] > 0) {
					(*MyOutput_) << "    " << std::hex << std::setw(2) << pattern << ": " << std::dec << pattern_count[posneg][delay][pattern] << std::endl;
				}
			}
		}
	}
...
void ChamberUtilities::CFEBTiming_with_Posnegs_basic_scan() {
	std::fstream clct_file("/home/cscme11/aschneid/me11/BasicScan_CLCTs.txt", std::ios_base::out);
	std::fstream out_file("/home/cscme11/aschneid/me11/BasicScan_Output.txt", std::ios_base::out);
	...
	bool is_inject_scan = true;
	int cfeb = 0;
	unsigned int layers = 0x1f;
	unsigned int pattern = 0xa;
	unsigned int half_strip = 16;

	int count[224]; memset(count, 0, sizeof(count));

	for(int posneg=0; posneg<2; ++posneg) {
		SetCfebRxPosNeg(posneg);
		config.cfeb_rx_posneg = posneg;
		for(int rx_delay=0; rx_delay<25; ++rx_delay) {
			SetCfebRxClockDelay(rx_delay);
			config.cfeb_rx_clock_delay = rx_delay;

			CFEBTiming_PulseInject(is_inject_scan, cfeb, layers, pattern, half_strip); // Pulse or inject
			thisTMB_->DecodeCLCT();
			++count[thisTMB_->GetCLCT0keyHalfStrip()];
			MyOutput_ = &clct_file;
			Print_CLCT0();
			MyOutput_ = &out_file;
...
void ChamberUtilities::CFEBTiming_with_Posnegs_simple_routine(bool is_inject_scan, int time_delay, int cfeb_num, unsigned int layers, unsigned int pattern, int halfstrip, bool print_data, int cfeb_clock_phase) {
	...
	// Configure the crate for the test
	//CFEBTiming_Configure();
	CFEBTiming_Configuration config;
	config.cfeb_pipeline_depth = 53;
	config.dac = (float) 160 * 5. / 4095.;
...
int ChamberUtilities::CFEBHalfStripToTMBHalfStrip(int cfeb, int halfstrip) {
	...
	//
	if ( thisTMB_->GetTMBFirmwareCompileType() == 0xa ) {   // normal TMB compile type
		//
		pulsed_halfstrip = cfeb*32 + halfstrip;
		//
	} else if ( thisTMB_->GetTMBFirmwareCompileType() == 0xc ) {  // plus endcap ME1/1
		//
		pulsed_halfstrip = cfeb*32 + halfstrip;
		if (cfeb == 4)
			pulsed_halfstrip = 159 - halfstrip;
		//
	} else if ( thisTMB_->GetTMBFirmwareCompileType() == 0xd ) {  // minus endcap ME1/1
...
void ChamberUtilities::CFEBTiming_with_Posnegs(CFEBTiming_scanType scanType) {
	...
	for (int posneg=0; posneg<2; posneg++) {
		...
		for (int TimeDelay=0; TimeDelay<MaxTimeDelay; TimeDelay++){
			...
			//
			thisTMB_->SetCfeb0RxClockDelay(TimeDelay); 		// Set delay in TMB object
			thisTMB_->WriteRegister(phaser_cfeb0_rxd_adr);	// Write delay to hardware
			thisTMB_->FirePhaser(phaser_cfeb0_rxd_adr);		// Load delay
			//
			thisTMB_->SetCfeb1RxClockDelay(TimeDelay);
			thisTMB_->WriteRegister(phaser_cfeb1_rxd_adr);
			thisTMB_->FirePhaser(phaser_cfeb1_rxd_adr);
			//
			thisTMB_->SetCfeb2RxClockDelay(TimeDelay);
			thisTMB_->WriteRegister(phaser_cfeb2_rxd_adr);
			thisTMB_->FirePhaser(phaser_cfeb2_rxd_adr);
			//
			thisTMB_->SetCfeb3RxClockDelay(TimeDelay);
			thisTMB_->WriteRegister(phaser_cfeb3_rxd_adr);
			thisTMB_->FirePhaser(phaser_cfeb3_rxd_adr);
			//
			thisTMB_->SetCfeb4RxClockDelay(TimeDelay);
			thisTMB_->WriteRegister(phaser_cfeb4_rxd_adr);
			thisTMB_->FirePhaser(phaser_cfeb4_rxd_adr);
			//
			//
			...
			for (int Nmuons=0; Nmuons<2; Nmuons++){
				...
				for (int List=0; List<5; List++){
					...
					//
					int clct0patternId    = thisTMB_->GetCLCT0PatternId();
					int clct0nhit         = thisTMB_->GetCLCT0Nhit();
					int clct0keyHalfStrip = thisTMB_->GetCLCT0keyHalfStrip();
					if (debug_) {
		...
		for (int posneg=0; posneg<2; posneg++) {
			...
			for (int TimeDelay=0; TimeDelay<MaxTimeDelay; TimeDelay++){
				...
				//{
					...
					usleep(50);
					//
					char hs[6]; memset(hs, 0, sizeof(hs));// hs[0-6] is 1-32 have strips in layers 0-6
					thisDMB_->Set_TMB_TX_MODE(List, 5);

					/*
					for (int l = 0; l < 6; l++)
					{
						char layer_mask = 1 << l;
						thisDMB->Set_TMB_TX_MODE_Layer_Mask(List, layer_mask);
						for (int i = 1; i < 33; i++)
						{
							hs[l] = i & 0x1f;
							thisDMB->Set_TMB_TX_MODE_ShiftLayers(List, hs);
							usleep(10000000);
							//thisDMB->inject(1, 0x00);
							thisDMB->inject(1, 0x4f); //Copied delay from original routine
							usleep(10000000);
						}
					}
					*/
					...
					char layer_mask = 1;
					thisDMB_->Set_TMB_TX_MODE_Layer_Mask(List, layer_mask);
					hs[1] = 1 & 0x1f;
					thisDMB_->Set_TMB_TX_MODE_ShiftLayers(List, hs);
					usleep(10000);
					//thisDMB->inject(1, 0x00);
					thisCCB_->inject(1, 0x4f); //Copied delay from original routine
					usleep(10000);

					thisDMB_->Set_TMB_TX_MODE(List, 0);
	...
	bool test_in_special_region =
			inSpecialRegion(test_cfeb_tof_delay[0],test_cfeb_tof_delay[1],test_cfeb_tof_delay[2],test_cfeb_tof_delay[3],test_cfeb_tof_delay[4],
					...
					scanType);
	//
	int initial_cfeb_rxd_int_delay[5] = { thisTMB_->GetCFEB0RxdIntDelay(),
			thisTMB_->GetCFEB1RxdIntDelay(),
			thisTMB_->GetCFEB2RxdIntDelay(),
			thisTMB_->GetCFEB3RxdIntDelay(),
			thisTMB_->GetCFEB4RxdIntDelay() };
	//
	...
	thisCCB_->setCCBMode(CCB::DLOG);
	//
	thisTMB_->SetClctPatternTrigEnable(initial_clct_pretrig_enable);
	thisTMB_->WriteRegister(seq_trig_en_adr);
	//
	thisTMB_->SetTmbAllowClct(initial_clct_trig_enable);
	thisTMB_->WriteRegister(tmb_trig_adr);
	//
	thisTMB_->SetHsPretrigThresh(initial_clct_halfstrip_pretrig_thresh);
	thisTMB_->SetMinHitsPattern(initial_clct_pattern_thresh);
	thisTMB_->WriteRegister(seq_clct_adr);
	//
	thisTMB_->SetIgnoreCcbStartStop(initial_ignore_ccb_startstop);
	thisTMB_->WriteRegister(ccb_trig_adr);
	//
	thisTMB_->SetEnableLayerTrigger(initial_layer_trig_enable);
	thisTMB_->WriteRegister(layer_trg_mode_adr);
	//
	if (use_measured_values_) {
		...
		//
		thisTMB_->SetCfeb0RxClockDelay(CFEBrxPhase_[0]);
		thisTMB_->SetCfeb0RxPosNeg(CFEBrxPosneg_[0]);
		thisTMB_->WriteRegister(phaser_cfeb0_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb0_rxd_adr);
		//
		thisTMB_->SetCfeb1RxClockDelay(CFEBrxPhase_[1]);
		thisTMB_->SetCfeb1RxPosNeg(CFEBrxPosneg_[1]);
		thisTMB_->WriteRegister(phaser_cfeb1_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb1_rxd_adr);
		//
		thisTMB_->SetCfeb2RxClockDelay(CFEBrxPhase_[2]);
		thisTMB_->SetCfeb2RxPosNeg(CFEBrxPosneg_[2]);
		thisTMB_->WriteRegister(phaser_cfeb2_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb2_rxd_adr);
		//
		thisTMB_->SetCfeb3RxClockDelay(CFEBrxPhase_[3]);
		thisTMB_->SetCfeb3RxPosNeg(CFEBrxPosneg_[3]);
		thisTMB_->WriteRegister(phaser_cfeb3_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb3_rxd_adr);
		//
		thisTMB_->SetCfeb4RxClockDelay(CFEBrxPhase_[4]);
		thisTMB_->SetCfeb4RxPosNeg(CFEBrxPosneg_[4]);
		thisTMB_->WriteRegister(phaser_cfeb4_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb4_rxd_adr);
		//
	} else {
		...
		//
		thisTMB_->SetCfeb0RxClockDelay(initial_cfeb_phase[0]);
		thisTMB_->SetCfeb0RxPosNeg(initial_cfeb_posneg[0]);
		thisTMB_->WriteRegister(phaser_cfeb0_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb0_rxd_adr);
		//
		thisTMB_->SetCfeb1RxClockDelay(initial_cfeb_phase[1]);
		thisTMB_->SetCfeb1RxPosNeg(initial_cfeb_posneg[1]);
		thisTMB_->WriteRegister(phaser_cfeb1_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb1_rxd_adr);
		//
		thisTMB_->SetCfeb2RxClockDelay(initial_cfeb_phase[2]);
		thisTMB_->SetCfeb2RxPosNeg(initial_cfeb_posneg[2]);
		thisTMB_->WriteRegister(phaser_cfeb2_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb2_rxd_adr);
		//
		thisTMB_->SetCfeb3RxClockDelay(initial_cfeb_phase[3]);
		thisTMB_->SetCfeb3RxPosNeg(initial_cfeb_posneg[3]);
		thisTMB_->WriteRegister(phaser_cfeb3_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb3_rxd_adr);
		//
		thisTMB_->SetCfeb4RxClockDelay(initial_cfeb_phase[4]);
		thisTMB_->SetCfeb4RxPosNeg(initial_cfeb_posneg[4]);
		thisTMB_->WriteRegister(phaser_cfeb4_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb4_rxd_adr);
		//
	}
	...
	//
	thisTMB_->RedirectOutput(MyOutput_);
	thisDMB_->RedirectOutput(MyOutput_);
	thisCCB_->RedirectOutput(MyOutput_);
	thisMPC_->RedirectOutput(MyOutput_);
	//
	return;
}
//
...
void ChamberUtilities::CFEBTiming_without_Posnegs(){
	...
	// Set up for this test...
	// Get initial values:
	int initial_clct_pretrig_enable           = thisTMB_->GetClctPatternTrigEnable();    //0x68
	int initial_clct_trig_enable              = thisTMB_->GetTmbAllowClct();             //0x86
	int initial_clct_halfstrip_pretrig_thresh = thisTMB_->GetHsPretrigThresh();          //0x70
	int initial_clct_pattern_thresh           = thisTMB_->GetMinHitsPattern();           //0x70
	int initial_layer_trig_enable             = thisTMB_->GetEnableLayerTrigger();       //0xf0
	int initial_ignore_ccb_startstop          = thisTMB_->GetIgnoreCcbStartStop();       //0x2c
	int initial_cfeb0_phase = thisTMB_->GetCfeb0RxClockDelay();                          //0x112
	int initial_cfeb1_phase = thisTMB_->GetCfeb1RxClockDelay();                          //0x114
	int initial_cfeb2_phase = thisTMB_->GetCfeb2RxClockDelay();                          //0x116
	int initial_cfeb3_phase = thisTMB_->GetCfeb3RxClockDelay();                          //0x118
	int initial_cfeb4_phase = thisTMB_->GetCfeb4RxClockDelay();                          //0x11a
	//
	...
	thisTMB_->WriteRegister(tmb_trig_adr);
	//
	thisTMB_->SetHsPretrigThresh(6);
	thisTMB_->SetMinHitsPattern(6);
	thisTMB_->WriteRegister(seq_clct_adr);
	//
	thisTMB_->SetIgnoreCcbStartStop(0);
	thisTMB_->WriteRegister(ccb_trig_adr);
	...
	for (int TimeDelay=0; TimeDelay<MaxTimeDelay; TimeDelay++){
		...
		//
		thisTMB_->SetCfeb0RxClockDelay(TimeDelay);
		thisTMB_->WriteRegister(phaser_cfeb0_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb0_rxd_adr);
		//
		thisTMB_->SetCfeb1RxClockDelay(TimeDelay);
	...
	if (use_measured_values_) {
		...
		(*MyOutput_) << "Reverting back to original cfeb[0-4]delay phase values..." << std::endl;
		//
		thisTMB_->SetCfeb0RxClockDelay(initial_cfeb0_phase);
		thisTMB_->WriteRegister(phaser_cfeb0_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb0_rxd_adr);
		//
		thisTMB_->SetCfeb1RxClockDelay(initial_cfeb1_phase);
		thisTMB_->WriteRegister(phaser_cfeb1_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb1_rxd_adr);
		//
		thisTMB_->SetCfeb2RxClockDelay(initial_cfeb2_phase);
		thisTMB_->WriteRegister(phaser_cfeb2_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb2_rxd_adr);
		//
		thisTMB_->SetCfeb3RxClockDelay(initial_cfeb3_phase);
		thisTMB_->WriteRegister(phaser_cfeb3_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb3_rxd_adr);
		//
		thisTMB_->SetCfeb4RxClockDelay(initial_cfeb4_phase);
		thisTMB_->WriteRegister(phaser_cfeb4_rxd_adr);
		thisTMB_->FirePhaser(phaser_cfeb4_rxd_adr);
		//
		//
	...
	thisTMB_->WriteRegister(vme_ddd1_adr);
	thisTMB_->WriteRegister(vme_ddd2_adr);
	::sleep(1);
	//
	comparing_with_clct_ = false;
...
int ChamberUtilities::Find_alct_rx_with_ALCT_to_TMB_evenodd(int number_of_passes) {
	...
	// send output to std::cout except for the essential information
	thisTMB_->RedirectOutput(&std::cout);
	thisALCT_->RedirectOutput(&std::cout);
	//
	// Get initial values
	int initial_alct_rx_phase      = thisTMB_->GetAlctRxClockDelay();
	//
	int initial_fire_l1a_oneshot  = thisTMB_->GetFireL1AOneshot();
	int initial_ignore_ccb_rx     = thisTMB_->GetIgnoreCCBRx();
	int initial_sequencer_command = thisTMB_->GetAlctSequencerCommand();
	int initial_demux_mode        = thisTMB_->GetAlctDemuxMode();
	int initial_ALCT_read_address = thisTMB_->GetAlctRawReadAddress();
	//
	int value = thisTMB_->ReadRegister(alct_sync_ctrl_adr);
	if (debug_>=10) std::cout << "begin routine (0x" << std::hex << alct_sync_ctrl_adr << ") = 0x" << value << std::endl;
	//
	// Set up for this test:
	// turn off the CCB inputs...
	thisTMB_->SetFireL1AOneshot(0);
	thisTMB_->SetIgnoreCCBRx(1);
	thisTMB_->WriteRegister(ccb_cfg_adr);
	//
	// put the ALCT into "transmit Teven/Todd mode"
	thisTMB_->SetAlctSequencerCommand(SEND_EVENODD);
	thisTMB_->WriteRegister(alct_cfg_adr);
	//
	// Error accumulators
	...
	for (int ipass=0; ipass<number_of_passes; ipass++) {
		for (int delay_value=0; delay_value<maximum_number_of_phase_delay_values; delay_value++) {
			...
			thisTMB_->SetAlctRxClockDelay(delay_value);
			thisTMB_->WriteRegister(phaser_alct_rxd_adr);
			thisTMB_->FirePhaser(phaser_alct_rxd_adr);
			//
			if (debug_>=10) std::cout << "Clear errors in flipflops..." << std::endl;
			thisTMB_->SetALCTSyncRxDataDelay(0);         //Set depth where to look for the data
			thisTMB_->SetALCTSyncClearErrors(1);         // Clear TMB data check flipflops
			thisTMB_->WriteRegister(alct_sync_ctrl_adr);
			//
			value = thisTMB_->ReadRegister(alct_sync_ctrl_adr);
			...
			thisTMB_->SetALCTSyncClearErrors(0);
			thisTMB_->WriteRegister(alct_sync_ctrl_adr);
			//
			value = thisTMB_->ReadRegister(alct_sync_ctrl_adr);
			...
			for (int i=0; i<=7; i++) {
				...
				thisTMB_->SetAlctDemuxMode(DEMUX_DATA);
				thisTMB_->SetAlctRawReadAddress(i);
				thisTMB_->WriteRegister(alctfifo1_adr);
				//
				if (debug_>=10) std::cout << "read data..." << std::endl;
				thisTMB_->ReadRegister(alctfifo2_adr);
				alct_demux_rd[i] = thisTMB_->GetReadAlctDemuxData();
			}
			...
			//
			int alct_sync_1st_err = thisTMB_->GetReadALCTSync1stError();
			int alct_sync_2nd_err = thisTMB_->GetReadALCTSync2ndError();
			//
			alct_sync_1st_err_ff[delay_value] |= thisTMB_->GetReadALCTSync1stErrorLatched();
			alct_sync_2nd_err_ff[delay_value] |= thisTMB_->GetReadALCTSync2ndErrorLatched();
	...
	thisTMB_->SetFireL1AOneshot(initial_fire_l1a_oneshot);
	thisTMB_->SetIgnoreCCBRx(initial_ignore_ccb_rx);
	thisTMB_->WriteRegister(ccb_cfg_adr);
	//
	thisTMB_->SetAlctSequencerCommand(initial_sequencer_command);
	thisTMB_->WriteRegister(alct_cfg_adr);
	//
	thisTMB_->SetAlctDemuxMode(initial_demux_mode);
	thisTMB_->SetAlctRawReadAddress(initial_ALCT_read_address);
	thisTMB_->WriteRegister(alctfifo1_adr);
	//
	if (use_measured_values_) {
		...
		thisTMB_->SetAlctRxClockDelay(ALCTrxPhase_);
	} else {
		...
		thisTMB_->SetAlctRxClockDelay(initial_alct_rx_phase);
	}
	thisTMB_->WriteRegister(phaser_alct_rxd_adr);
	thisTMB_->FirePhaser(phaser_alct_rxd_adr);
	//
...
int ChamberUtilities::Find_alct_tx_with_TMB_to_ALCT_evenodd(int number_of_passes) {
	...
	//
	// Get initial values
	int initial_alct_tx_phase      = thisTMB_->GetAlctTxClockDelay();
	//
	int initial_fire_l1a_oneshot  = thisTMB_->GetFireL1AOneshot();
	int initial_ignore_ccb_rx     = thisTMB_->GetIgnoreCCBRx();
	int initial_sequencer_command = thisTMB_->GetAlctSequencerCommand();
	...
	for (int ipass=0; ipass<number_of_passes; ipass++) {
		for (int delay_value=0; delay_value<maximum_number_of_phase_delay_values; delay_value++) {
			...
			thisTMB_->SetAlctTxClockDelay(delay_value);
			thisTMB_->WriteRegister(phaser_alct_txd_adr);
			thisTMB_->FirePhaser(phaser_alct_txd_adr);
			//
			...
			for (int bank=0; bank<3; bank++) {
				...
				//
				thisTMB_->SetAlctSequencerCommand(register_address);
				thisTMB_->WriteRegister(alct_cfg_adr);
				//
				// first 80MHz frame
				thisTMB_->SetALCTSyncTxData1st(data_to_loopback_1st[bank]);
				thisTMB_->WriteRegister(alct_sync_txdata_1st_adr);
				//
				// second 80MHz frame
				thisTMB_->SetALCTSyncTxData2nd(data_to_loopback_2nd[bank]);
				thisTMB_->WriteRegister(alct_sync_txdata_2nd_adr);
			}
			//
			//Set depth where to look for the data
			thisTMB_->SetALCTSyncRxDataDelay(0);
			//
			// Clear TMB data check flipflops
			thisTMB_->SetALCTSyncClearErrors(1);
			thisTMB_->WriteRegister(alct_sync_ctrl_adr);
			//
			// Unclear error flipflops, after this write, the errors are being tallied by TMB firmware
			thisTMB_->SetALCTSyncClearErrors(0);
			...
			//
			// Read TMB data check flipflops
			thisTMB_->ReadRegister(alct_sync_ctrl_adr);
			//
			int alct_sync_1st_err = thisTMB_->GetReadALCTSync1stError();
			int alct_sync_2nd_err = thisTMB_->GetReadALCTSync2ndError();
			//
	...
	if (use_measured_values_) {
		...
		thisTMB_->SetAlctTxClockDelay(ALCTtxPhase_);
	} else {
		...
		thisTMB_->SetAlctTxClockDelay(initial_alct_tx_phase);
	}
	thisTMB_->WriteRegister(phaser_alct_txd_adr);
	thisTMB_->FirePhaser(phaser_alct_txd_adr);
	//
...
int ChamberUtilities::ALCT_TMB_TimingUsingRandomLoopback() {
	...
	// Get initial values
	int initial_alct_tx_phase      = thisTMB_->GetAlctTxClockDelay();
	int initial_alct_tx_posneg     = thisTMB_->GetAlctTxPosNeg();
	int initial_alct_rx_phase      = thisTMB_->GetAlctRxClockDelay();
	int initial_alct_rx_posneg     = thisTMB_->GetAlctRxPosNeg();
	//
	int initial_fire_l1a_oneshot  = thisTMB_->GetFireL1AOneshot();
	int initial_ignore_ccb_rx     = thisTMB_->GetIgnoreCCBRx();
	//
	int initial_sequencer_command   = thisTMB_->GetAlctSequencerCommand();
	//
	int initial_alct_sync_rx_data_delay = thisTMB_->GetALCTSyncRxDataDelay();
	int initial_alct_sync_tx_random     = thisTMB_->GetALCTSyncTXRandom();
	//
	int initial_demux_mode        = thisTMB_->GetAlctDemuxMode();
	int initial_ALCT_read_address = thisTMB_->GetAlctRawReadAddress();
	//
	...
	thisTMB_->WriteRegister(ccb_cfg_adr);
	//
	thisTMB_->SetAlctSequencerCommand(LOOPBACK_RANDOM);
	thisTMB_->WriteRegister(alct_cfg_adr);
	//
	...
	for (int rx_posneg=0; rx_posneg<2; rx_posneg++) {
		//
		thisTMB_->SetAlctRxPosNeg(rx_posneg);
		thisTMB_->WriteRegister(phaser_alct_rxd_adr);
		thisTMB_->FirePhaser(phaser_alct_rxd_adr);
		//
		for (int tx_posneg=0; tx_posneg<2; tx_posneg++) {
			//
			thisTMB_->SetAlctTxPosNeg(tx_posneg);
			thisTMB_->WriteRegister(phaser_alct_txd_adr);
			thisTMB_->FirePhaser(phaser_alct_txd_adr);
			//
			for (int pipe_depth=min_pipedepth; pipe_depth<max_pipedepth; pipe_depth++) {
				...
				for (int rx_value=0; rx_value<maximum_number_of_phase_delay_values; rx_value++) {
					thisTMB_->SetAlctRxClockDelay(rx_value);
					thisTMB_->WriteRegister(phaser_alct_rxd_adr);
					thisTMB_->FirePhaser(phaser_alct_rxd_adr);
					//
					for (int tx_value=0; tx_value<maximum_number_of_phase_delay_values; tx_value++) {
						thisTMB_->SetAlctTxClockDelay(tx_value);
						thisTMB_->WriteRegister(phaser_alct_txd_adr);
						thisTMB_->FirePhaser(phaser_alct_txd_adr);
						//
						//Set depth where to look for the data
						thisTMB_->SetALCTSyncRxDataDelay(pipe_depth);
						//
						thisTMB_->SetALCTSyncTXRandom(1);
						//
						// Clear TMB data check flipflops
						thisTMB_->SetALCTSyncClearErrors(1);
						thisTMB_->WriteRegister(alct_sync_ctrl_adr);
						...
						thisTMB_->ReadRegister(alct_sync_ctrl_adr);
						//
						if ( thisTMB_->GetReadALCTSync1stErrorLatched()==0 && thisTMB_->GetReadALCTSync2ndErrorLatched()==0 ) {
	...
	if (good_depth < 0) {
		...
		thisTMB_->WriteRegister(ccb_cfg_adr);
		//
		thisTMB_->SetALCTSyncRxDataDelay(initial_alct_sync_rx_data_delay);
		thisTMB_->SetALCTSyncTXRandom(initial_alct_sync_tx_random);
		thisTMB_->WriteRegister(alct_sync_ctrl_adr);
		//
		thisTMB_->SetAlctSequencerCommand(initial_sequencer_command);
		thisTMB_->WriteRegister(alct_cfg_adr);
		...
		//
		thisTMB_->SetAlctRxClockDelay(initial_alct_rx_phase);
		thisTMB_->SetAlctRxPosNeg(initial_alct_rx_posneg);
		thisTMB_->WriteRegister(phaser_alct_rxd_adr);
		thisTMB_->FirePhaser(phaser_alct_rxd_adr);
		//
		thisTMB_->SetAlctTxClockDelay(initial_alct_tx_phase);
		thisTMB_->SetAlctTxPosNeg(initial_alct_tx_posneg);
		thisTMB_->WriteRegister(phaser_alct_txd_adr);
		thisTMB_->FirePhaser(phaser_alct_txd_adr);
		//
		return -1;
	...
	// bit to be 1... Thus.....
	//
	int alct_tof_delay = thisTMB_->GetAlctTOFDelay();
	//
	...
	if (use_measured_values_) {
		...
		thisTMB_->SetAlctTxClockDelay(ALCTtxPhase_);
		thisTMB_->SetAlctTxPosNeg(ALCTtxPosNeg_);
		thisTMB_->SetAlctRxClockDelay(ALCTrxPhase_);
		thisTMB_->SetAlctRxPosNeg(ALCTrxPosNeg_);
		//
	} else {
	...
	//
	thisTMB_->RedirectOutput(MyOutput_);
	thisALCT_->RedirectOutput(MyOutput_);
	//
	return ALCTtxPhase_;
}
//
...
void ChamberUtilities::ThoroughRxTxScan(int rx_posneg, int tx_posneg, int pipedepth) {
	...
	thisTMB_->FirePhaser(phaser_alct_txd_adr);
	//
	thisTMB_->SetALCTSyncRxDataDelay(pipedepth);
	thisTMB_->WriteRegister(alct_sync_ctrl_adr);
	//
	//
	...
	for (int rx_value=0; rx_value<maximum_number_of_phase_delay_values; rx_value++) {
		...
		for (int tx_value=0; tx_value<maximum_number_of_phase_delay_values; tx_value++) {
			...
			for (int check_loop=0; check_loop<max_loop; check_loop++) {
				...
				if (thisTMB_->GetReadALCTSync1stErrorLatched() || thisTMB_->GetReadALCTSync2ndErrorLatched()) check_loop = max_loop+1;
				//
			}
			//
...
int ChamberUtilities::ALCT_TMB_TimingUsingErrorCorrectionCode() {
	...
	int initial_ignore_ccb_rx     = thisTMB_->GetIgnoreCCBRx();
	//
	int initial_ecc_enable          = thisTMB_->GetALCTErrorCorrectionCodeEnable();
	int initial_sequencer_command   = thisTMB_->GetAlctSequencerCommand();
	//
	int initial_alct_sync_rx_data_delay = thisTMB_->GetALCTSyncRxDataDelay();
	int initial_alct_sync_tx_random     = thisTMB_->GetALCTSyncTXRandom();
	...
	thisTMB_->SetALCTErrorCorrectionCodeEnable(1);
	thisTMB_->WriteRegister(alct_stat_adr);
	//
	int good_depth = -999;
	//
	// A display (statistics) array...
	...
	for (int pipe_depth=0; pipe_depth<16; pipe_depth++) {
		...
		//
		thisTMB_->SetALCTSyncRxDataDelay(pipe_depth);
		thisTMB_->SetALCTSyncTXRandom(0);
		thisTMB_->WriteRegister(alct_sync_ctrl_adr);
		//
		for (int rx_value=0; rx_value<maximum_number_of_phase_delay_values; rx_value++) {
			thisTMB_->SetAlctRxClockDelay(rx_value);
			...
			for (int tx_value=0; tx_value<maximum_number_of_phase_delay_values; tx_value++) {
				...
				thisTMB_->SetAlctSequencerCommand(NORMAL_MODE);
				thisTMB_->WriteRegister(alct_cfg_adr);
				//
				// Tell ALCT to send 49-bits of random data (+ 7-bit Error Correction Code data)
				thisTMB_->SetAlctSequencerCommand(SEND_RANDOM);
				thisTMB_->WriteRegister(alct_cfg_adr);
				//
				...
				for (int check_loop=0; check_loop<10; check_loop++) {
					...
					thisTMB_->ReadRegister(alct_sync_ctrl_adr);
					//
					number_of_latch_errors += (thisTMB_->GetReadALCTSync1stErrorLatched() | thisTMB_->GetReadALCTSync2ndErrorLatched());
					//
					// no sense wasting time on this rx,tx value.  It has badness...
					if (thisTMB_->GetReadALCTSync1stErrorLatched() || thisTMB_->GetReadALCTSync2ndErrorLatched()) go_quick = true;
					//
					// OK for this read:  read it again, slowly...
					if (!thisTMB_->GetReadALCTSync1stErrorLatched() && !thisTMB_->GetReadALCTSync2ndErrorLatched()) go_quick = false;
					//
				}
				//
				thisTMB_->GetCounters();
				int tx_one_bit_errors           = thisTMB_->GetCounter( thisTMB_->GetECCTriggerPathOneErrorCounterIndex() );
				int tx_two_bit_errors           = thisTMB_->GetCounter( thisTMB_->GetECCTriggerPathTwoErrorsCounterIndex() );
				int tx_more_than_two_bit_errors = thisTMB_->GetCounter( thisTMB_->GetECCTriggerPathMoreThanTwoErrorsCounterIndex() );
				//
	...
	ALCT_phase_analysis(alct_tx_rx_analyze);
	//
	ALCTrxPosNeg_ = thisTMB_->GetAlctRxPosNeg();
	...
	thisTMB_->WriteRegister(alct_cfg_adr);
	//
	thisTMB_->SetALCTErrorCorrectionCodeEnable(initial_ecc_enable);
	thisTMB_->WriteRegister(alct_stat_adr);
	//
	thisTMB_->SetAlctDemuxMode(initial_demux_mode);
	thisTMB_->SetAlctRawReadAddress(initial_ALCT_read_address);
		...
		for (int ifirstsecond=0; ifirstsecond<2; ifirstsecond++) {
			for (int itransmit=0; itransmit<28; itransmit++) {
				...
				for (int bank=0; bank<3; bank++) {
					...
					//
					// first 80MHz frame
					thisTMB_->SetALCTSyncTxData1st(alct_1st_phase[bank]);
					thisTMB_->WriteRegister(alct_sync_txdata_1st_adr);
					//
					// second 80MHz frame
					thisTMB_->SetALCTSyncTxData2nd(alct_2nd_phase[bank]);
					thisTMB_->WriteRegister(alct_sync_txdata_2nd_adr);
					//
...
int ChamberUtilities::ALCTBC0Scan() {
	...
	//
	// Get initial values
	int initial_fire_l1a_one_shot = thisTMB_->GetFireL1AOneshot();
	int initial_ignore_ccb_rx     = thisTMB_->GetIgnoreCCBRx();
	//
	int initial_mpc_idle_blank     = thisTMB_->GetMpcIdleBlank();
	int initial_mpc_sel_ttc_bx0    = thisTMB_->GetSelectMpcTtcBx0();
	//
	int initial_clct_bx0_delay     = thisTMB_->GetClctBx0Delay();
	int initial_alct_bx0_delay     = thisTMB_->GetAlctBx0Delay();
	int initial_alct_bx0_enable    = thisTMB_->GetAlctBx0Enable();
	//
	int initial_alct_txdata_delay  = thisTMB_->GetALCTTxDataDelay();
	//
	int initial_bxn_offset         = thisTMB_->GetBxnOffset();
	//
	//
	if (debug_>=10) {
	...
	std::cout    << "alct_tof_delay          = 0x" << std::hex << thisTMB_->GetAlctTOFDelay() << std::endl;
	std::cout    << "tmb_to_alct_data_delay  = 0x" << std::hex << thisTMB_->GetALCTTxDataDelay() << std::endl;
	(*MyOutput_) << "This scan has the following input parameters... " << std::endl;
	(*MyOutput_) << "alct_tof_delay          = 0x" << std::hex << thisTMB_->GetAlctTOFDelay() << std::endl;
	(*MyOutput_) << "tmb_to_alct_data_delay  = 0x" << std::hex << thisTMB_->GetALCTTxDataDelay() << std::endl;
	//
	// Set up for this test:
	// turn off the one shot L1A (from TMB)...
	thisTMB_->SetFireL1AOneshot(0);
	// turn on the CCB inputs to get BC0 defined from CCB
	thisTMB_->SetIgnoreCCBRx(0);
	thisTMB_->WriteRegister(ccb_cfg_adr);
	if (debug_>=10) {
		thisTMB_->ReadRegister(ccb_cfg_adr);
		thisTMB_->PrintTMBRegister(ccb_cfg_adr);
	}
	//
	// MpcIdleBlank = 0 = send BC0 signals to MPC every orbit (1 = blank unless LCT is sent to MPC)
	thisTMB_->SetMpcIdleBlank(0);
	// SelectMpcTtcBx0 = 0 = send BC0 to MPC (on LCT0) from TMB (1 = send BC0 from CCB)
	thisTMB_->SetSelectMpcTtcBx0(0);
	thisTMB_->WriteRegister(tmb_trig_adr);
	if (debug_>=10) {
		thisTMB_->ReadRegister(tmb_trig_adr);
		thisTMB_->PrintTMBRegister(tmb_trig_adr);
	}
	//
	// AlctBx0Enable = 1 = send BC0 to MPC (on LCT1) from ALCT (0 = copy what it has from the TMB)
	thisTMB_->SetAlctBx0Enable(1);
	thisTMB_->WriteRegister(bx0_delay_adr);
	if (debug_>=10) {
		thisTMB_->ReadRegister(bx0_delay_adr);
		thisTMB_->PrintTMBRegister(bx0_delay_adr);
	}
	//
	...
	if (initial_bxn_offset != local_tmb_bxn_offset_) {
		thisTMB_->SetBxnOffset(local_tmb_bxn_offset_);
		thisTMB_->WriteRegister(seq_offset_adr);
		//
	...
	for (int delay_value=minimum_delay_value; delay_value<maximum_delay_value; delay_value++) {
		//
		thisTMB_->SetAlctBx0Delay(delay_value);
		thisTMB_->WriteRegister(bx0_delay_adr);
		if (debug_>=10) {
			thisTMB_->ReadRegister(bx0_delay_adr);
			thisTMB_->PrintTMBRegister(bx0_delay_adr);
		...
		for (int j=0; j<number_of_checks_per_value; j++) {
			thisTMB_->ReadRegister(bx0_delay_adr);
			int BC0_match = thisTMB_->GetReadBx0Match();
			//
			if (debug_>=10 && BC0_match)
				std::cout << "delay " << thisTMB_->GetBxnOffset() << " ---> ALCT*CLCT BC0_match = " << BC0_match << "<-----" << std::endl;
			//
	...
	thisTMB_->SetFireL1AOneshot(initial_fire_l1a_one_shot);
	thisTMB_->SetIgnoreCCBRx(initial_ignore_ccb_rx);
	thisTMB_->WriteRegister(ccb_cfg_adr);
	//
	thisTMB_->SetSelectMpcTtcBx0(initial_mpc_sel_ttc_bx0);
	thisTMB_->SetMpcIdleBlank(initial_mpc_idle_blank);
	thisTMB_->WriteRegister(tmb_trig_adr);
	//
	thisTMB_->SetClctBx0Delay(initial_clct_bx0_delay);
	thisTMB_->SetAlctBx0Delay(initial_alct_bx0_delay);
	thisTMB_->SetAlctBx0Enable(initial_alct_bx0_enable);
	thisTMB_->WriteRegister(bx0_delay_adr);
	//
	thisTMB_->SetALCTTxDataDelay(initial_alct_txdata_delay);
	thisTMB_->WriteRegister(alct_stat_adr);
	//
	if (use_measured_values_) {
		(*MyOutput_) << "Setting alct_bx0_delay and match_trig_alct_delay to measured value..." << std::endl;
		//
		thisTMB_->SetAlctBx0Delay(ALCT_bx0_delay_);
		thisTMB_->WriteRegister(bx0_delay_adr);
		//
		thisTMB_->SetAlctVpfDelay(match_trig_alct_delay_);
		thisTMB_->WriteRegister(tmbtim_adr);
		//
	} else {
		...
		if (initial_bxn_offset != local_tmb_bxn_offset_) {
			//
			thisTMB_->SetBxnOffset(initial_bxn_offset);
			thisTMB_->WriteRegister(seq_offset_adr);
			//
			// Send a BGo "Resync" to make sure that the BC0 offset has taken effect
			thisCCB_->setCCBMode(CCB::VMEFPGA);
...
int ChamberUtilities::RatTmbDelayScan() {
	...
	//                                [2]=loop_tmb
	//                                [3]=free_tx0
	int initial_data = thisTMB_->ReadRegister(vme_ratctrl_adr);
	int write_data = initial_data & 0xfffe | 0x0001;
	thisTMB_->WriteRegister(vme_ratctrl_adr,write_data);
	//
	//enable RAT input into TMB...
	int read_data = thisTMB_->ReadRegister(rpc_inj_adr);
	write_data = read_data | 0x0001;
	thisTMB_->WriteRegister(rpc_inj_adr,write_data);
	//
	...
	for (int pass=0; pass<=100; pass++) { //collect statistics
		...
		for (int ddd_delay=0; ddd_delay<16; ddd_delay++) {
			...
			for (int irat=0; irat<=1; irat++) {           //only 2 RPCs exist
				//
				read_data = thisTMB_->ReadRegister(rpc_cfg_adr);
				read_data &= 0xf9ff;                        //zero out old RAT bank
				write_data = read_data | (irat << 9);       //select RAT RAM bank
				thisTMB_->WriteRegister(rpc_cfg_adr,write_data);
				//
				read_data = thisTMB_->ReadRegister(rpc_cfg_adr);
				int rpc_rbxn = (read_data >> 11) & 0x0007;  //RPC MSBS for sync mode
				//
				int rpc_rdata = thisTMB_->ReadRegister(rpc_rdata_adr) & 0xffff; //RPC RAM read data for sync mode (LSBS)
				//
	...
	thisTMB_->WriteRegister(vme_ratctrl_adr,initial_data);
	//
	if (use_measured_values_) {
...
int ChamberUtilities::FindWinner(){
	...
	// Set up for this test...
	// Get initial values:
	int initial_mpc_output_enable = thisTMB_->GetMpcOutputEnable();
	int initial_mpc_rx_delay      = thisTMB_->GetMpcRxDelay();
	//
	if (debug_>=10) std::cout << "Set mpc_output_enable = 1" << std::endl;
	thisTMB_->SetMpcOutputEnable(1);
	thisTMB_->WriteRegister(tmb_trig_adr);
	::sleep(1);
	//
	...
	for (int delay_value=delaymin; delay_value<=delaymax; delay_value++) {
		...
		//
		thisTMB_->SetMpcRxDelay(delay_value);
		thisTMB_->WriteRegister(tmb_trig_adr);
		//
		thisTMB_->ResetCounters();
		//
		...
		//
		thisTMB_->GetCounters();
		number_of_mpc_accepted[delay_value] = thisTMB_->GetCounter( thisTMB_->GetLCTAcceptedByMPCCounterIndex() );
		//
		if (debug_) {
			std::cout << ", number of MPC winner bits accepted = "
					<< std::dec << number_of_mpc_accepted[delay_value] << std::endl;
		}
		if (debug_>10) thisTMB_->PrintCounters();
	}
	//
	...
	//
	thisTMB_->SetMpcOutputEnable(initial_mpc_output_enable);
	if (use_measured_values_) {
		(*MyOutput_) << "Setting measured value of mpc_rx_delay..." << std::endl;
		thisTMB_->SetMpcRxDelay(MPCdelay_);
	} else {
		(*MyOutput_) << "Reverting back to original value of mpc_rx_delay..." << std::endl;
		thisTMB_->SetMpcRxDelay(initial_mpc_rx_delay);
	}
	thisTMB_->WriteRegister(tmb_trig_adr);
	//
	thisTMB_->RedirectOutput(MyOutput_);
...
int ChamberUtilities::FindWinner(int npulses){
	...
	//
	// Get initial values:
	int initial_mpc_output_enable             = thisTMB_->GetMpcOutputEnable();          //0x86
	int initial_clct_trig_enable              = thisTMB_->GetTmbAllowClct();             //0x86
	int initial_mpc_rx_delay                  = thisTMB_->GetMpcRxDelay();               //0x86
	int initial_layer_trig_enable             = thisTMB_->GetEnableLayerTrigger();       //0xf0
	int initial_clct_pretrig_enable           = thisTMB_->GetClctPatternTrigEnable();    //0x68
	int initial_clct_halfstrip_pretrig_thresh = thisTMB_->GetHsPretrigThresh();          //0x70
	int initial_clct_pattern_thresh           = thisTMB_->GetMinHitsPattern();           //0x70
	...
	if (debug_) {
		std::cout << "After enabling, TMB register 0x" << std::hex << tmb_trig_adr
				<< " = 0x" << thisTMB_->ReadRegister(tmb_trig_adr)
				<< " which has value of mpc_output_enable = " << thisTMB_->GetMpcOutputEnable() << std::endl;
	}
	//
	float MpcDelay=0;
	int   MpcDelayN=0;
	...
	for (int i = 0; i < DelaySize; i++){
		...
		while (iterations < npulses) {
			...
			if (debug_) {
				...
				thisMPC_->read_csr0();
				//
				thisTMB_->DataSendMPC();
				thisTMB_->GetCounters();
				thisTMB_->PrintCounters();
				//
				std::cout << "----------> Accepted at TMB:  MPC0 = " << thisTMB_->MPC0Accept()
		  		<< ", MPC1 = " << thisTMB_->MPC1Accept() << std::endl ;
			}
			//
			if ( (thisTMB_->MPC0Accept()+thisTMB_->MPC1Accept()) > 0 ) {
				MpcDelay  += i ;
				MpcDelayN++;
				if ( thisTMB_->MPC0Accept() > 0 ) {
					Mpc0Delay  += i ;
					Mpc0DelayN++;
					MPC0Count[i]++ ;
				}
				if ( thisTMB_->MPC1Accept() > 0 ) {
					Mpc1Delay  += i ;
					Mpc1DelayN++;
					MPC1Count[i]++ ;
				}
...
int ChamberUtilities::MeasureAlctDavCableDelay() {
	...
	// Get initial values:
	int initial_mpc_output_enable = thisTMB_->GetMpcOutputEnable();
	int initial_delay_value = thisDMB_->GetAlctDavCableDelay();
	//
	...
	for (int delay=DelayMin; delay<=DelayMax; delay++) {
		//
		// Set the delay value:
		thisDMB_->SetAlctDavCableDelay(delay);         //insert the delay value into the DMB cable_delay_ parameter
		thisDMB_->setcbldly(thisDMB_->GetCableDelay()); //write the value
		::sleep(1);
		//
		// Get the data:
		ZeroDmbHistograms();
	...
	if (use_measured_values_) {
		...
		thisDMB_->SetAlctDavCableDelay(AlctDavCableDelay_);
	} else {
		(*MyOutput_) << "Reverting back to original value of alct_dav_cable_delay..." << std::endl;
		thisDMB_->SetAlctDavCableDelay(initial_delay_value);
	}
	thisDMB_->setcbldly(thisDMB_->GetCableDelay());
	//
...
int ChamberUtilities::MeasureTmbLctCableDelay() {
	...
	int current_xlatency = thisDMB_->GetxLatency();
	...
	// Enable this TMB to send LCTs to MPC:
	int initial_mpc_output_enable = thisTMB_->GetMpcOutputEnable();
	int initial_delay_value = thisDMB_->GetTmbLctCableDelay();
	//
	thisTMB_->SetMpcOutputEnable(1);
	thisTMB_->WriteRegister(tmb_trig_adr);
	//
	...
	for (int delay=DelayMin; delay<=DelayMax; delay++) {
		...
		//
		// Set the delay value:
		thisDMB_->SetTmbLctCableDelay(delay);        //insert the delay value into the DMB cable_delay_ parameter
		int cable_delay = thisDMB_->GetCableDelay(); //get DMB::cable_delay_
		thisDMB_->setcbldly(cable_delay);            //write the value
		::sleep(1);
		//
	...
	if (use_measured_values_) {
		...
		thisDMB_->SetTmbLctCableDelay(TmbLctCableDelay_);
	} else {
		(*MyOutput_) << "Reverting back to original value of tmb_lct_cable_delay..." << std::endl;
		thisDMB_->SetTmbLctCableDelay(initial_delay_value);
	}
	thisDMB_->setcbldly(thisDMB_->GetCableDelay());
	//
...
int ChamberUtilities::MeasureCfebDavCableDelay() {
	...
	// Get initial values:
	int initial_mpc_output_enable = thisTMB_->GetMpcOutputEnable();
	int initial_delay_value = thisDMB_->GetCfebDavCableDelay();
	//
	// Enable this TMB to send LCTs to MPC:
	thisTMB_->SetMpcOutputEnable(1);
	thisTMB_->WriteRegister(tmb_trig_adr);
	...
	for (int delay=DelayMin; delay<=DelayMax; delay++) {
		//
		// Set the delay value:
		thisDMB_->SetCfebDavCableDelay(delay);         //insert the delay value into the DMB cable_delay_ parameter
		thisDMB_->setcbldly(thisDMB_->GetCableDelay()); //write the value
		::sleep(1);
		//
		// Get the data:
	...
	if (use_measured_values_) {
		...
		thisDMB_->SetCfebDavCableDelay(CfebDavCableDelay_);
	} else {
		(*MyOutput_) << "Reverting back to original value of cfeb_dav_cable_delay..." << std::endl;
		thisDMB_->SetCfebDavCableDelay(initial_delay_value);
	}
	thisDMB_->setcbldly(thisDMB_->GetCableDelay());
	//
...
void ChamberUtilities::SetupRadioactiveTriggerConditions() {
	//
	initial_clct_pretrig_thresh_ = thisTMB_->GetHsPretrigThresh();
	initial_clct_pattern_thresh_ = thisTMB_->GetMinHitsPattern();
	initial_alct_pretrig_thresh_ = thisALCT_->GetPretrigNumberOfLayers();
	initial_alct_pattern_thresh_ = thisALCT_->GetPretrigNumberOfPattern();
	//
	thisTMB_->SetHsPretrigThresh(1);
	thisTMB_->SetMinHitsPattern(1);
	thisTMB_->WriteRegister(seq_clct_adr);
	//
	thisALCT_->SetPretrigNumberOfLayers(1);
	thisALCT_->SetPretrigNumberOfPattern(1);
	thisALCT_->WriteConfigurationReg();
	//
	return;
}
//
void ChamberUtilities::ReturnToInitialTriggerConditions() {
	...
	thisTMB_->SetHsPretrigThresh(initial_clct_pretrig_thresh_);
	thisTMB_->SetMinHitsPattern(initial_clct_pattern_thresh_);
	thisTMB_->WriteRegister(seq_clct_adr);
	//
	thisALCT_->SetPretrigNumberOfLayers(initial_alct_pretrig_thresh_);
	thisALCT_->SetPretrigNumberOfPattern(initial_alct_pattern_thresh_);
	thisALCT_->WriteConfigurationReg();
	//
	return;
}
...
int ChamberUtilities::FindTmbAndAlctL1aDelay(){
	...
	// Get initial values:
	int initial_mpc_output_enable = thisTMB_->GetMpcOutputEnable();
	int initial_tmb_delay             = thisTMB_->GetL1aDelay();
	int initial_alct_l1a_delay        = thisALCT_->GetWriteL1aDelay();
	//
	// Enable this TMB for this test
	thisTMB_->SetMpcOutputEnable(1);
	thisTMB_->WriteRegister(tmb_trig_adr);
	...
	for (int delay=0; delay<=range; delay++){
		...
		//
		thisTMB_->SetL1aDelay(tmb_delay_value);
		thisTMB_->WriteRegister(seq_l1a_adr);
		//
		thisALCT_->SetL1aDelay(alct_delay_value);
		thisALCT_->WriteConfigurationReg();
		if (debug_>=10) {
			thisALCT_->ReadConfigurationReg();
			thisALCT_->PrintConfigurationReg();
		}
		//
		thisTMB_->ResetCounters();    // reset counters
		//
		::sleep(getPauseAtEachSetting());                  // accumulate statistics
		//
		thisTMB_->GetCounters();      // read counter values
		//
		tmb_in_l1a_window[tmb_delay_value]   = thisTMB_->GetCounter( thisTMB_->GetL1AInTMBWindowCounterIndex() );
		alct_in_l1a_window[alct_delay_value] = thisTMB_->GetCounter( thisTMB_->GetALCTRawHitsReadoutCounterIndex()  );
		//
		if (debug_) {
	...
	if (use_measured_values_) {
		...
		thisTMB_->SetL1aDelay(TMBL1aTiming_);
		thisALCT_->SetL1aDelay(ALCTL1aDelay_);
	} else {
		(*MyOutput_) << "Reverting back to original value of tmb(alct)_l1a_delay..." << std::endl;
		thisTMB_->SetL1aDelay(initial_tmb_delay);
		thisALCT_->SetL1aDelay(initial_alct_l1a_delay);
	}
	thisTMB_->WriteRegister(seq_l1a_adr);
	thisALCT_->WriteConfigurationReg();
	//
...
int ChamberUtilities::FindTMB_L1A_delay(int delay_min, int delay_max){
	...
	// Get initial values:
	int initial_mpc_output_enable = thisTMB_->GetMpcOutputEnable();
	int initial_delay             = thisTMB_->GetL1aDelay();
	//
	// Enable this TMB for this test
	thisTMB_->SetMpcOutputEnable(1);
	thisTMB_->WriteRegister(tmb_trig_adr);
	...
	for (int delay=delay_min; delay<=delay_max; delay++){
		...
		thisTMB_->GetCounters();      // read counter values
		//
		tmb_in_l1a_window[delay] = thisTMB_->GetCounter( thisTMB_->GetL1AInTMBWindowCounterIndex() );
		//
	...
	if (use_measured_values_) {
		...
		thisTMB_->SetL1aDelay(initial_delay);
	}
	thisTMB_->WriteRegister(seq_l1a_adr);
	//
	thisTMB_->RedirectOutput(MyOutput_);
...
int ChamberUtilities::FindALCT_L1A_delay(int minlimit, int maxlimit){
	...
	for (int delay=minlimit; delay<=maxlimit; delay++) {
		...
		thisTMB_->GetCounters();
		//
		ALCT_l1a_accepted[delay] = thisTMB_->GetCounter( thisTMB_->GetALCTRawHitsReadoutCounterIndex() );
		//
...
void ChamberUtilities::ALCTChamberScan(){
	...
	//
	// read initial values:
	int initial_alct_trigger_mode    = thisALCT_->GetTriggerMode();
	int initial_alct_pat_trig_enable = thisTMB_->GetAlctPatternTrigEnable();
	//
	// Set up for this test:
	thisTMB_->SetAlctPatternTrigEnable(1);
	thisTMB_->WriteRegister(seq_trig_en_adr);
	//
	thisALCT_->SetTriggerMode(0);
	thisALCT_->WriteConfigurationReg();
	if (debug_>=10) {
		thisALCT_->ReadConfigurationReg();
		thisALCT_->PrintConfigurationReg();
	...
	for (int Ninject=0; Ninject<NPulses; Ninject++){
		//
		for (int keyWG=0; keyWG<(thisALCT_->GetNumberOfChannelsInAlct())/6; keyWG++) {
			//
			if (debug_) std::cout << "Injecting in WG = " << std::dec << keyWG << std::endl;
			//
			for(int layer=0; layer<MAX_NUM_LAYERS; layer++) {
				for(int channel=0; channel<(thisALCT_->GetNumberOfChannelsInAlct())/6; channel++) {
					if (channel==keyWG) {
						thisALCT_->SetHotChannelMask(layer,channel,ON);
					} else {
						thisALCT_->SetHotChannelMask(layer,channel,OFF);
					}
				}
			}
			//
			thisALCT_->WriteHotChannelMask();
			if (debug_>=10) {
				thisALCT_->ReadHotChannelMask();
				thisALCT_->PrintHotChannelMask();
			}
			//
			PulseTestStrips();
			::usleep(100);
			...
			if (debug_>=10) {
				thisTMB_->PrintALCT();
				std::cout << "ALCT Wordcount = " << std::dec << thisTMB_->GetALCTWordCount() << std::endl;
			}
			//
			if (thisTMB_->GetAlct0FirstKey() == keyWG &&
					thisTMB_->GetAlct0Quality()  >= 1 )
				chamberResult[keyWG]++;
			//
			if (thisTMB_->GetAlct1SecondKey()== keyWG &&
					thisTMB_->GetAlct1Quality()  >= 1 )
				chamberResult2[keyWG]++;
			//
			InJected[keyWG]++;
		}
	}
	for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++)
		ALCTWireScan_[keyWG] = chamberResult[keyWG];
	//
	(*MyOutput_) << "Wire" << std::endl;
	for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) (*MyOutput_) << keyWG/100 ;
	(*MyOutput_) << std::endl;
	for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) (*MyOutput_) << ((keyWG/10)%10) ;
	(*MyOutput_) << std::endl;
	for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) (*MyOutput_) << keyWG%10 ;
	(*MyOutput_) << std::endl;
	(*MyOutput_) << "Injected" << std::endl;
	for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) (*MyOutput_) << InJected[keyWG] ;
	(*MyOutput_) << std::endl;
	(*MyOutput_) << "ALCT Chamber Scan Result" << std::endl;
	for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) (*MyOutput_) << chamberResult[keyWG] ;
	(*MyOutput_) << std::endl;
	for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) (*MyOutput_) << chamberResult2[keyWG] ;
	(*MyOutput_) << std::endl;
	//
	if (debug_) {
		std::cout << "Wire" << std::endl;
		for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) std::cout << keyWG/100 ;
		std::cout << std::endl;
		for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) std::cout << ((keyWG/10)%10) ;
		std::cout << std::endl;
		for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) std::cout << keyWG%10 ;
		std::cout << std::endl;
		std::cout << "Injected" << std::endl;
		for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) std::cout << InJected[keyWG] ;
		std::cout << std::endl;
		std::cout << "ALCT Chamber Scan Result" << std::endl;
		for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) std::cout << chamberResult[keyWG] ;
		std::cout << std::endl;
		for (int keyWG=0; keyWG<thisALCT_->GetNumberOfChannelsPerLayer(); keyWG++) std::cout << chamberResult2[keyWG] ;
		std::cout << std::endl;
	}
	//
	// revert back to initial settings
	thisTMB_->SetAlctPatternTrigEnable(initial_alct_pat_trig_enable);
	thisTMB_->WriteRegister(seq_trig_en_adr);
	//
	thisALCT_->SetTriggerMode(initial_alct_trigger_mode);
	thisALCT_->WriteConfigurationReg();
	//
	thisTMB_->RedirectOutput(MyOutput_);
	thisDMB_->RedirectOutput(MyOutput_);
...
void ChamberUtilities::FindDistripHotChannels(){
	...
	for (int layer=0; layer<number_of_layers; layer++)
		for (int distrip=0; distrip<number_of_distrips_per_layer; distrip++)
			initial_hot_channel_mask[layer][distrip] = thisTMB_->GetDistripHotChannelMask(layer,distrip);
	//
	...
	for (int layer=0; layer<number_of_layers; layer++) {
		for (int distrip=0; distrip<number_of_distrips_per_layer; distrip++) {
			...
			thisTMB_->SetDistripHotChannelMask(layer,distrip,hot_channel_mask[layer][distrip]);
		}
	}
	thisTMB_->WriteDistripHotChannelMasks();
	::usleep(10);
	//
	...
	thisTMB_->GetCounters();
	//
	int clct_pretrigger_counts = thisTMB_->GetCounter( thisTMB_->GetCLCTPretriggerCounterIndex() );
		...
		for (int distrip_test=0; distrip_test<number_of_distrips_per_layer; distrip_test++) {
			...
			for (int layer=0; layer<number_of_layers; layer++) {
				for (int distrip=0; distrip<number_of_distrips_per_layer; distrip++) {
					...
					thisTMB_->SetDistripHotChannelMask(layer,distrip,working_hot_channel_mask[layer][distrip]);
				}
			}
			thisTMB_->WriteDistripHotChannelMasks();
			::usleep(10);
			...
			thisTMB_->GetCounters();
			//
			clct_pretrigger_counts = thisTMB_->GetCounter( thisTMB_->GetCLCTPretriggerCounterIndex() );
			if (debug_) std::cout << "---> CLCT pretrigger rate = " << clct_pretrigger_counts << std::endl;
			//
	...
	for (int layer=0; layer<number_of_layers; layer++) {
		...
		thisTMB_->packCharBuffer(hot_channel_mask[layer],
	...
	for (int layer=0; layer<number_of_layers; layer++) {
		for (int distrip=0; distrip<number_of_distrips_per_layer; distrip++) {
			thisTMB_->SetDistripHotChannelMask(layer,distrip,initial_hot_channel_mask[layer][distrip]);
		}
	}
	thisTMB_->WriteDistripHotChannelMasks();
	//
...
void ChamberUtilities::InitSystem(){
	...
	//
	thisCCB_->configure();
	if (thisTMB_) thisTMB_->configure();
	if (thisDMB_) thisDMB_->configure();
	if (thisMPC_) thisMPC_->configure();
	if (thisALCT_)    thisALCT_->configure();
	//
...
void ChamberUtilities::InitStartSystem(){
	...
	if (thisALCT_)    thisALCT_->configure();
	//
	if (thisTMB_) thisTMB_->StartTTC();
	if (thisTMB_) thisTMB_->EnableL1aRequest();
	thisCCB_->setCCBMode(CCB::VMEFPGA);      // It needs to be in FPGA mod to work.
	//
}
//
...
void ChamberUtilities::SetupCoincidencePulsing(int amplitude, int pulse_signal) {
	...
	for (int layer=0; layer<6; layer++) {
		thisTMB_->SetDistripHotChannelMask(layer,0,0);
		thisTMB_->SetDistripHotChannelMask(layer,39,0);
	}
	thisTMB_->WriteDistripHotChannelMasks();
	//
	if (debug_) {
		thisTMB_->ReadDistripHotChannelMasks();
		thisTMB_->PrintHotChannelMask();
	}
	//
	...
	thisTMB_->SetEnableAllCfebsActive(1);
	thisTMB_->WriteRegister(seq_trig_en_adr);
	//
	//
	// Set up ALCT teststrip pulsing:
	//
	if (debug_) std::cout << "Setup coincidence pulsing in TMB slot " << thisTMB_->slot() << std::endl;
	const int strip_mask = 0x3f;        //pulse all six (layers of) strips
	//
	thisALCT_->SetUpPulsing(amplitude,PULSE_LAYERS,strip_mask,pulse_signal);
	::sleep(1);
	//
	UsePulsing_ = true;
	//
...
void ChamberUtilities::PulseRandomALCT(int delay){
	//
	thisALCT_->SetUpRandomALCT();
	//
	thisTMB_->ResetALCTRAMAddress();
	PulseTestStrips(delay);
	printf("Decode ALCT\n");
	thisTMB_->DecodeALCT();
	//
	printf(" WordCount %d \n",thisTMB_->GetALCTWordCount());
	//
	(*MyOutput_) << std::endl;
	//
...
void ChamberUtilities::PulseTestStrips(int delay){
	...
	if ( thisALCT_ ) {
		...
		if ( beginning == 0 ) {
			...
			int amplitude = 255;
			int strip_mask = 0xff;
			thisALCT_->SetUpPulsing(amplitude,PULSE_AFEBS,strip_mask,ADB_ASYNC);
			//
			beginning = 1;
			//
			PulseTestStrips();
			...
			//
			if (debug_>=10) {
				if (thisALCT_->GetChamberType().find("ME11")!= std::string::npos) {
					std::cout << thisALCT_->GetChamberType().find("ME11") <<std::endl;
					std::cout << thisALCT_->GetChamberType() <<std::endl;
...
//
void ChamberUtilities::PulseCFEB(int * HalfStrips, int CLCTInputs, bool enableL1aEmulator ){
	//
...
void ChamberUtilities::InjectMPCData(){
	...
	for (int i=0; i<DelaySize; i++) {
		...
		for (int npulse=0; npulse<1; npulse++) {
			...
			thisMPC_->read_fifos();
			//
			thisTMB_->ReadRegister(0x86);
			thisTMB_->ReadRegister(0x92);
			thisTMB_->ReadRegister(0x90);
			//
			if ( thisTMB_->MPC0Accept() > 0 ) {
				Mpc0Delay  += i ;
				Mpc0DelayN++;
...
void ChamberUtilities::PopulateDmbHistograms() {
	...
	for (int i=0; i<getNumberOfDataReads(); i++) {
		...
		//
		thisDMB_->readtimingCounter();
		thisDMB_->readtimingScope();
		//
		int AffToL1aValue = thisDMB_->GetL1aLctCounter();
		int CfebDavValue  = thisDMB_->GetCfebDavCounter();
		int TmbDavValue   = thisDMB_->GetTmbDavCounter();
		int AlctDavValue  = thisDMB_->GetAlctDavCounter();
		//
		int AffToL1aScope = thisDMB_->GetL1aLctScope();
		int CfebDavScope  = thisDMB_->GetCfebDavScope();
		int TmbDavScope   = thisDMB_->GetTmbDavScope();
		int AlctDavScope  = thisDMB_->GetAlctDavScope();
		//
...
int ChamberUtilities::AdjustL1aLctDMB(){
	//
	thisDMB_->readtimingCounter();
	if ( thisDMB_->GetL1aLctCounter() == 0 ) {
		printf(" ****************************** \n");
	...
	for( int l1a=90; l1a<110; l1a++) {
		...
		::sleep(2);
		thisDMB_->readtimingCounter();
		Counter = thisDMB_->GetL1aLctCounter() ;
...
void ChamberUtilities::ALCTScanDelays(){
	...
	//
	thisTMB_->SetALCTPatternTrigger();
	//
	for ( int nloop=0; nloop<20; nloop++){
		for ( int DelaySetting=0; DelaySetting<20; DelaySetting++){
			//
			int keyWG  = int(rand()/(RAND_MAX+0.01)*thisALCT_->GetNumberOfChannelsPerLayer());
			...
			thisALCT_->PrintHotChannelMask();
			//
			for (int afeb=0; afeb<thisALCT_->GetNumberOfAfebs(); afeb++)
				thisALCT_->SetAsicDelay(afeb,DelaySetting);
			thisALCT_->WriteAsicDelaysAndPatterns();
			//
			PulseTestStrips();
			printf("Decode ALCT\n");
			thisTMB_->DecodeALCT();
			...
			// Now analyze
			//
			if ( (thisTMB_->GetAlct0Quality()   != alct0_quality) ||
					(thisTMB_->GetAlct1Quality()   != alct1_quality) ||
					(thisTMB_->GetAlct0FirstBxn()  != alct0_bxn) ||
					(thisTMB_->GetAlct1SecondBxn() != alct1_bxn) ||
					(thisTMB_->GetAlct0FirstKey()  != alct0_bxn) ||
					(thisTMB_->GetAlct1SecondKey() != alct1_bxn) ) {
				//
				alct0_quality     = thisTMB_->GetAlct0Quality();
				alct1_quality     = thisTMB_->GetAlct1Quality();
				alct0_bxn         = thisTMB_->GetAlct0FirstBxn();
				alct1_bxn         = thisTMB_->GetAlct1SecondBxn();
				alct0_key         = thisTMB_->GetAlct0FirstKey();
				alct1_key         = thisTMB_->GetAlct1SecondKey();
				//
