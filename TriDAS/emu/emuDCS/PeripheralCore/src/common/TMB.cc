...
std::ostream & operator<<(std::ostream & os, TMB & tmb) {
  ...
  os << std::dec << "TMB: crate " << tmb.theCrate_
     ...
     << "l1a window size " << tmb.read_l1a_window_size_ << std::endl
     << "l1a delay " << tmb.read_l1adelay_ << std::endl
...
void TMB::SetTMBRegisterDefaults() {
  ...
  //------------------------------------------------------------------
  read_l1adelay_         = l1adelay_default        ;
  read_l1a_window_size_  = l1a_window_size_default ;
  read_tmb_l1a_internal_ = tmb_l1a_internal_default;
  //
  //------------------------------------------------------------------
...
void TMB::PrintTMBRegister(unsigned long int address) {
  ...
  //
    ...
    //
		...
		//--------------------------------------------------------------
		//[0X11A] = ADR_PHASER7:  CFEB5 -> TMB communication clock delay
		//--------------------------------------------------------------
		...
		//--------------------------------------------------------------
		//[0X11A] = ADR_PHASER8:  CFEB6 -> TMB communication clock delay
		//--------------------------------------------------------------
...
int TMB::FillTMBRegister(unsigned long int address) {
  ...
  if ( address == vme_loopbk_adr ) {
    ...
    //0X74 = ADR_SEQ_L1A:  Sequencer L1A configuration
    //------------------------------------------------------------------
    InsertValueIntoDataWord(read_l1adelay_        ,l1adelay_bithi        ,l1adelay_bitlo        ,&data_word);
    InsertValueIntoDataWord(read_l1a_window_size_ ,l1a_window_size_bithi ,l1a_window_size_bitlo ,&data_word);
    InsertValueIntoDataWord(read_tmb_l1a_internal_,tmb_l1a_internal_bithi,tmb_l1a_internal_bitlo,&data_word);
    //
  } else if ( address == seq_offset_adr ) {
    //------------------------------------------------------------------
    //0X76 = ADR_SEQ_OFFSET:  Sequencer Counter Offsets
...
void TMB::CheckTMBConfiguration(int max_number_of_reads) {
  ...
  while (!config_ok && number_of_configuration_reads_ < max_number_of_reads) {
    ...
    //0X74 = ADR_SEQ_L1A:  Sequencer L1A configuration
    //------------------------------------------------------------------
    config_ok &= compareValues("TMB tmb_l1a_delay"                             ,read_l1adelay_        ,read_l1adelay_        , print_errors);
    config_ok &= compareValues("TMB tmb_l1a_window_size"                       ,read_l1a_window_size_ ,read_l1a_window_size_ , print_errors);
    config_ok &= compareValues("TMB Generate internal L1a (overrides external)",read_tmb_l1a_internal_,read_tmb_l1a_internal_, print_errors);
    //
    //------------------------------------------------------------------
    //0X76 = ADR_SEQ_OFFSET:  Sequencer Counter Offsets
    //------------------------------------------------------------------
