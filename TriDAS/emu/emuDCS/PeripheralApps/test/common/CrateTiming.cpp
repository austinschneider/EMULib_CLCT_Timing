//-----------------------------------------------------------------------
// $Id: CrateTiming.cpp,v 1.1 2007/12/26 11:24:53 liu Exp $
// $Log: CrateTiming.cpp,v $
// Revision 1.1  2007/12/26 11:24:53  liu
...
emu::pc::TMB *thisTMB = 0 ;
emu::pc::DAQMB* thisDMB ;
emu::pc::CCB* thisCCB ;
...
int main(int argc,char **argv){
  ...
  EmuEndcap *myendcap = emuController.GetEmuEndcap();
  vector<Crate*> crateVector = myendcap->crates();
  if (crateVector.size() > 1){
    cerr << "Error: only one PeripheralCrate allowed" << endl;
    exit(1);
  }
  //
  vector<TMB*> tmbVector = myendcap->tmbs(crateVector[0]);
  if (tmbVector.size() > 1){
    cerr << "Error: only one TMB in xml file (" << xmlFile 
	 << ") allowed" << endl;
    exit(1);
  }
  vector<DAQMB*> dmbVector = myendcap->daqmbs(crateVector[0]);
  if (dmbVector.size() > 1){
