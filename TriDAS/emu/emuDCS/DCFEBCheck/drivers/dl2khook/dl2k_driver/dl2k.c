...
rio_probe1 (struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	void *ring_space;
	dma_addr_t ring_dma;
        printk(KERN_INFO " ENTER AND STARTED LSD \n");
	if (!version_printed++)
		printk ("%s", version);
...
receive_packet (struct net_device *dev)
{
	...
	int cnt = 30;
	//	printk(KERN_INFO " enter receive_packet \n"); 
	/* If RFDDone, FrameStart and FrameEnd set, there is a new packet in. */
	while (1) {
		struct netdev_desc *desc = &np->rx_ring[entry];
		int pkt_len;
		u64 frame_status;
                if(!(desc->status & RFDDone))break;
		//if(!(desc->status & FrameStart)){printk(KERN_INFO "FrameStart Problem \n");} 
		// if(!(desc->status & FrameEnd)){printk(KERN_INFO "FrameEnd Problem \n");}
				        
		/* Chip omits the CRC. */
		pkt_len = le64_to_cpu (desc->status & 0xffff);
		frame_status = le64_to_cpu (desc->status);
		//	printk(KERN_INFO " lsd framestat %08x %d \n",frame_status,pkt_len);
		if (--cnt < 0)
			break;
		/* Update rx error statistics, drop packet. */
		if ((frame_status & (RFS_Errors & 0x000F0000))||(pkt_len>MAX_JUMBO)) { // lsd kludge
		  //  printk(KERN_INFO " framestat %08x \n",frame_status);
			np->stats.rx_errors++;
			...
			struct sk_buff *skb;
                        np->stats.rx_packets++; // lsd kludge
			np->stats.rx_bytes=np->stats.rx_bytes+pkt_len; // lsd kludge
...
#endif
			// netif_rx (skb);
			// printk(KERN_INFO " call netif_rx_hook \n"); 
                        netif_rx_hook_2(skb);
			dev->last_rx = jiffies;
		}
