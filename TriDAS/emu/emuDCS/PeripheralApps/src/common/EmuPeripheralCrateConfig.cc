...
void EmuPeripheralCrateConfig::ChamberTests(xgi::Input * in, xgi::Output * out ) 
  ...
  *out << cgicc::form() << std::endl ;
  ...
  if ( (alct->GetChamberType()).find("ME11") != std::string::npos ) {
    ...
    *out << cgicc::form() << std::endl ;
    //
    std::string CFEBTimingDirect = toolbox::toString("/%s/CFEBTimingDirect",getApplicationDescriptor()->getURN().c_str());
		*out << cgicc::form().set("method","GET").set("action",CFEBTimingDirect) << std::endl ;
		*out << cgicc::input().set("type","submit").set("value","Scan CFEB rx phases") << std::endl ;
		sprintf(buf,"%d",tmb);
		*out << cgicc::input().set("type","hidden").set("value",buf).set("name","tmb");
		sprintf(buf,"%d",dmb);
  ...
  *out << cgicc::tr();
	...
	*out << cgicc::td().set("ALIGN", "left") << std::endl;
		*out << cgicc::select().set("name", "is_inject_scan") << std::endl;
			if((alct->GetChamberType()).find("ME11") != std::string::npos)
				*out << cgicc::option().set("value", "1") << "Inject" << cgicc::option() << std::endl;
			*out << cgicc::option().set("value", "0") << "Pulse" << cgicc::option() << std::endl;
		*out << cgicc::select() << std::endl;
	*out << cgicc::td();
	*out << cgicc::td().set("ALIGN", "left") << std::endl;
	...
	*out << cgicc::td().set("ALIGN", "left") << std::endl;
		*out << cgicc::select().set("name", "pattern") << std::endl;
			for(int i=0x2; i<0xb; ++i) *out << cgicc::option().set("value", toolbox::toString("%d", i)) << toolbox::toString("0x%x", i) << cgicc::option() << std::endl;
		*out << cgicc::select() << std::endl;
	*out << cgicc::td();
	*out << cgicc::td().set("ALIGN", "left") << std::endl;
	...
	*out << cgicc::input().set("type","submit").set("value","CFEBTimingL1AWindowScan") << std::endl;
	*out << " Print data:";
	*out << cgicc::input().set("type", "checkbox").set("name", "print_data").set("checked", "checked") << std::endl;
	*out << " Print clct:";
	*out << cgicc::input().set("type", "checkbox").set("name", "print_clct").set("checked", "checked") << std::endl;
	sprintf(buf,"%d",tmb);
	*out << cgicc::input().set("type","hidden").set("value",buf).set("name","tmb");
	sprintf(buf,"%d",dmb);
	*out << cgicc::input().set("type","hidden").set("value",buf).set("name","dmb");
...
void EmuPeripheralCrateConfig::CFEBTimingSimpleScan(xgi::Input * in, xgi::Output * out )
  ...
  name = cgi.getElement("is_inject_scan");
	if(name != cgi.getElements().end()) {
		is_inject_scan = cgi["is_inject_scan"]->getIntegerValue();
	}
	//
...
void EmuPeripheralCrateConfig::CFEBTimingL1AWindowScan(xgi::Input * in, xgi::Output * out ) throw (xgi::exception::Exception) {
	...
	bool print_data = false;
	bool print_clct = false;

	//
	name = cgi.getElement("print_data");
	if(name != cgi.getElements().end()) {
		print_data = cgi["print_data"]->getIntegerValue();
		std::cout << "print_data: " << print_data << std::endl;
	}
	//
	name = cgi.getElement("print_clct");
	if(name != cgi.getElements().end()) {
		print_clct = cgi["print_clct"]->getIntegerValue();
		std::cout << "print_clct: " << print_clct << std::endl;
	}
	//
	MyTest[tmb][current_crate_].RedirectOutput(&ChamberTestsOutput[tmb][current_crate_]);
...
void EmuPeripheralCrateConfig::CFEBTimingSetRxDelay(xgi::Input * in, xgi::Output * out ) throw (xgi::exception::Exception) {

}

void EmuPeripheralCrateConfig::CFEBTimingSetTOF(xgi::Input * in, xgi::Output * out ) throw (xgi::exception::Exception) {
